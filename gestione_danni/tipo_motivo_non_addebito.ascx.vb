...
Partial Class gestione_danni_tipo_motivo_non_addebito
    ...
    Private Sub Visibilita(Valore As DivVisibile)
        div_elenco_tipo_motivo_non_addebito.Visible = Valore And DivVisibile.Elenco

        div_modifica_tipo_motivo_non_addebito.Visible = Valore And DivVisibile.Modifica

        div_intestazione.Visible = Valore And DivVisibile.Intestazione
    End Sub

    Public Sub InitForm(Optional mio_record As veicoli_motivo_non_addebito_rds = Nothing, Optional stato As Integer = 0, Optional provenienza As String = "")
        lb_stato.Text = stato
    ...
    Protected Sub FillEdit(mio_record As veicoli_motivo_non_addebito_rds)
        With mio_record
            lb_id.Text = .id
    ...
    Protected Sub AzzeraEdit()
        Dim mio_record As veicoli_motivo_non_addebito_rds = New veicoli_motivo_non_addebito_rds
        With mio_record
            .id = 0
            .descrizione = ""
        End With
    ...
    Protected Sub listViewElenco_ItemCommand(sender As Object, e As System.Web.UI.WebControls.ListViewCommandEventArgs) Handles listViewElenco.ItemCommand
        ...
        ElseIf e.CommandName = "Elimina" Then
            ...
            If veicoli_motivo_non_addebito_rds.CancellaRecord(Integer.Parse(id_riga.Text)) Then

                listViewElenco.DataBind()

                Libreria.genUserMsgBox(Page, "Tipo motivo non addebito danno eliminato correttamente")
            Else
                Libreria.genUserMsgBox(Page, "Errore nell'eliminazione del tipo motivo non addebito danno.")
            End If
        End If
    End Sub
