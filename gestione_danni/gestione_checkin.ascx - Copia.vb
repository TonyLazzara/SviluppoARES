...
Partial Class gestione_danni_gestione_checkin
    ...
    Private Sub InvioMail(ByVal NumDoc, ByVal NumeroDiTarga, ByVal TipoDocumento)
        ...
        Try
            ...
            Dim StringaEmailDestinatari As String = "flotta@sicilyrentcar.it,it-support@sicilyrentcar.it"
            ...
            If Not RsSelect.HasRows Then 'Nessun danno presente                
                ...
                Try
                    ...
                    Sql = "insert into veicoli_evento_apertura_danno (attivo,sospeso_rds,id_veicolo,id_tipo_documento_apertura,id_documento_apertura,num_crv,data,data_creazione,id_utente) " & _
                                                              "values('0','0','" & lb_id_veicolo.Text & "','" & lb_id_tipo_documento_apertura.Text & "','" & lb_id_documento_apertura.Text & "','" & lb_num_crv.Text & "', CONVERT(date, '" & DataOggi & "', 102)" & ", CONVERT(datetime, '" & DataOggiConOrario & "', 102)" & ",'" & CInt(Request.Cookies("SicilyRentCar")("IdUtente")) & "')"
    ...
    Private Sub Visibilita_2(Valore As DivVisibile)
        ...
        'Tony 24-04-2023
        SqlRDSMailInviate.SelectCommand = "SELECT * FROM [RDS_mail_inviate] WITH(NOLOCK) where id_evento =' " & lb_id_evento.Text & "' ORDER BY data desc"
        listViewMailInviate.DataBind()
        'Response.Write("ID e: " & lb_id_evento.Text & "<br>")
        'Response.Write("Controllo " & ControlloInvioMailRDS(lb_id_evento.Text) & "<br>")
        'If lb_id_evento.Text <> "0" Then
        '    If ControlloInvioMailRDS(lb_id_evento.Text) Then
        '        btnInviaMail.BackColor = Drawing.Color.Green
        '    Else
        '        btnInviaMail.BackColor = System.Drawing.ColorTranslator.FromHtml("#e88532")
        '    End If
        'End If
        'FINE Tony
   ...
   Dim Dbc As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
    ...
    Public Sub InitFormRDS(ByVal id_tipo_documento As tipo_documento, ByVal id_documento As Integer, ByVal numero_crv As Integer)        
        'Response.Write(" Utilty: " & Integer.Parse(lb_id_tipo_documento_apertura.Text) & " - " & lb_id_documento_apertura.Text & " - " & lb_num_crv.Text)
        ...
        'Tony 04-05-2023 Utilty Manuale
        'Response.Write("InitFormRDS: " & Integer.Parse(id_tipo_documento) & " - " & id_documento & " - " & numero_crv)
        'Response.Write(" Id Evento" & lb_id_evento.Text)
        Select Case lb_id_evento.Text
            Case Is = "102609"
                lb_id_evento.Text = "102611"
            Case Is = 0
                Select Case id_documento
                    Case Is = "132300620"
                        lb_id_evento.Text = "98504"
                    Case Is = "402301808"
                        lb_id_evento.Text = "93159"
                End Select
        ...
        Dim Sql As String = "select * from veicoli_evento_apertura_danno WITH(NOLOCK) WHERE (id_documento_apertura = '" & lb_num_documento.Text & "') AND (attivo = 1) "
        'Response.Write(Sql)
        'Response.End()
        ...
        Try
            ...
            If Rs.HasRows Then
                Do While Rs.Read
                    If Rs("da_riparare") = True Then
                        chkDaRiparare.Checked = True
                    Else
                        chkDaRiparare.Checked = False
                    End If
                Loop
            End If
            Rs.Close()
            Rs = Nothing
        Catch ex As Exception
            Libreria.genUserMsgBox(Page, "Salvataggio DA RIPARARE ARES Errore contattare amministratore del sistema.")
        End Try
            ...
            With mio_record
                ...
                Select Case mio_stato_rds
                    ...
                    Case sessione_danni.stato_rds.Da_addebitare
                        ...
                        bt_salva_rds.Text = "Salva Modifiche"
                        'FINE Tony
                    Case sessione_danni.stato_rds.Da_fatturare
                        'Dim body As HtmlGenericControl = Page.Master.FindControl("myBody")
                        'body.Attributes.Add("onLoad", "InitDivVisibile(1);")
                        ...
                        Dim body As HtmlGenericControl = Page.Master.FindControl("myBody")
                        body.Attributes.Add("onLoad", "InitDivVisibile(1);")
                        DropOperazione.SelectedValue = 6

                        AbilitaRDS(False)
                        'DropOperazione.Visible = False
                        'lb_per_operazione.Visible = False
                        lb_operazione.Visible = True
                        lb_operazione.Text = "  ADDEBITATO"
    ...
    Protected Sub InviaFile(ByVal MioFileUpload As FileUpload, ByVal filePath As String)
        ...
        If MioFileUpload.HasFile Then
            ...
            Else
                Messaggio = "L'estensione del file deve essere con estensione (jpg, png o pdf)"
            End If
    ...
    'Tony 30-04-2023
    Protected Sub InviaFileNew(ByVal MioFileUpload As FileUpload, ByVal filePath As String)
        Dim Messaggio As String = ""
        If MioFileUpload.HasFile Then
            Dim estensione As String = LCase(Right(MioFileUpload.FileName, 4))
            If estensione = ".jpg" Or estensione = ".png" Or estensione = ".pdf" Then
                ...
                If MioFileUpload.PostedFile.ContentLength <= 5000000 Then
                    ...
                    If File.Exists(filePath & fileNameBig) Then                        
                        Try
                            ...
                            Dim SqlQuery As String
                            Dim DateSplit(1) As String
                            Dim Ora1 As DateTime
                            Dim Ora2 As DateTime
                            Dim OrePermessi As String
                            ...
                            'Response.Write(CDate(tt) & "<br>")
                            'Response.End()

                            Dim Sigla_Allegato As String = funzioni_comuni_new.getSiglaAllegato(DropDownTipoDocumentoImg.SelectedValue, "veicoli_danni_img_tipo_documenti")
                            Dim DesNomeFile As String = ""
                            DesNomeFile = Sigla_Allegato & "_" & yy & "_" & lb_id_evento.Text & estensione

                            SqlInsert = "insert into veicoli_danni_evento_foto (id_evento,data_creazione,tipo_documento,descrizione,riferimento_foto, operatore) "
                            SqlInsert = SqlInsert & " values('" & Integer.Parse(lb_id_evento.Text) & "', CONVERT(DATETIME, '" & tt & "', 102),'" & DropDownTipoDocumentoImg.SelectedValue & "','" & DesNomeFile & "','" & fileNameBig & "','" & Request.Cookies("SicilyRentCar")("nome") & "')"
                            
                            'Response.Write("SQL: " & SqlInsert & "<br>")
                            'Response.End()

                            Cmd3 = New Data.SqlClient.SqlCommand(SqlInsert, Dbc3)
                            Cmd3.ExecuteNonQuery()

                            SqlQuery = "insert into query_log (data,utente,query) values('" & Now & "','" & Request.Cookies("SicilyRentCar")("nome") & "','" & Replace(SqlInsert, "'", "''") & "')"
                            Cmd3.CommandText = SqlQuery
                            'Response.Write(Cmd3.CommandText & "<br/>")
                            'Response.End()
                            Cmd3.ExecuteNonQuery()
                            ...
                            Dbc3 = Nothing
                        Catch ex As Exception
                            Libreria.genUserMsgBox(Page, ex.ToString & " Salvataggio Dati Errore contattare amministratore del sistema.")
                        End Try
    ...
    Protected Sub btnInviaFile_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnInviaFile.Click
        ...
        InviaFileNew(FileUpload1, HttpContext.Current.Request.MapPath(HttpContext.Current.Request.ApplicationPath) & "\images\DocDanni\")
    End Sub

    'TONY 24-04-2023    
    Protected Sub btnInvioMail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnInviaMail.Click        
        'Response.Write("Evento: " & DropDownTipoEventoAperturaDanno.SelectedValue)
        If DropDownTipoEventoAperturaDanno.SelectedValue = 1 Then 'Chiusura Contratto
            'Allegati
            'myMail.Attachments.Add(New Attachment(allegato))

            Dim AllegatiSelezionati As Boolean = False

            For Each elemento In listViewDocumenti.Items
                If CType(elemento.FindControl("chkAllegatoEmail"), CheckBox).Checked = True Then
                    AllegatiSelezionati = True
                    Dim FileConPercorso As String = "D:\inetpub\wwwroot\ares.sicilyrentcar.it\images\DocDanni\" & CType(elemento.FindControl("lb_riferimento_foto"), Label).Text
                    'Response.Write(FileConPercorso & "<br>")
                    'Response.End()

                    If System.IO.File.Exists(FileConPercorso) Then
                    Else                        
                        Libreria.genUserMsgBox(Page, "Attenzione!! File/s selezionato/i non presente/i nella cartella: " & FileConPercorso)
                    End If
                End If
            Next

            If AllegatiSelezionati Then
                Dim StringaEmailMittente As String = "noreply@sicilyrentcar.it"
                Dim StringaEmailDestinatari As String
                ...
                Try
                    ...
                    myMail = New MailMessage()
                    myMail.From = New MailAddress(StringaEmailMittente)


                    'myMail.To.Add("alazzara@inwind.it")
                    Try
                        Dim Dbc As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
                        Dbc.Open()

                        Dim Cmd As New Data.SqlClient.SqlCommand("SELECT contratti_conducenti.id_conducente, contratti_conducenti.COGNOME, contratti_conducenti.nome, contratti_conducenti.EMAIL, contratti_conducenti.cell,contratti_conducenti.nazione FROM contratti_conducenti WITH(NOLOCK) WHERE num_contratto='" & lb_num_documento.Text & "' and num_conducente=1", Dbc)
                        'Response.Write(Cmd.CommandText & "<br><br>")
                        'Response.End()
                        ...
                        If Rs.HasRows Then
                            Do While Rs.Read
                                StringaEmailDestinatari = Rs("email")
                                If Rs("nazione") = "16" Then
                                    nazionalita = "IT"
                                Else
                                    nazionalita = "EE"
                                End If
                    ...
                    Catch ex As Exception
                        Libreria.genUserMsgBox(Page, ex.Message & " Carica Dati email --- Errore contattare amministratore del sistema.")
                    End Try


                    myMail.To.Add(StringaEmailDestinatari)
                    'myMail.To.Add("tonyboyscoutlzz@gmail.com")

                    'myMail.Bcc.Add("it-support@sicilyrentcar.it")
                    myMail.Bcc.Add("it-support@sicilyrentcar.it,flotta@sicilyrentcar.it")

                    If nazionalita = "IT" Then
                        myMail.Subject = "Contratto di Noleggio n. " & lb_num_documento.Text
                    Else
                        myMail.Subject = "Rental Agreement nr. " & lb_num_documento.Text
                    End If


                    'Allegati
                    'myMail.Attachments.Add(New Attachment(allegato))
                    Dim ArrayAllegatiInviati As String
                    Try                        
                        For Each elemento In listViewDocumenti.Items
                            If CType(elemento.FindControl("chkAllegatoEmail"), CheckBox).Checked = True Then
                                Dim NomeAllegato = CType(elemento.FindControl("lb_descrizione"), Label).Text
                                Dim FileConPercorso2 As String = "D:\inetpub\wwwroot\ares.sicilyrentcar.it\images\DocDanni\" & CType(elemento.FindControl("lb_riferimento_foto"), Label).Text
                                'Response.Write(FileConPercorso2 & "<br>")
                                'Response.End()
                                myMail.Attachments.Add(New Attachment(FileConPercorso2))
                                ArrayAllegatiInviati = ArrayAllegatiInviati & NomeAllegato & " / "
                            End If
                        Next
                    Catch ex As Exception
                        Libreria.genUserMsgBox(Page, ex.Message & " Errore generico allegati --- Errore contattare amministratore del sistema.")
                    End Try
                    ...
                    If nazionalita = "IT" Then
                        myMail.Body = "Gentile Cliente,<br><br>"
                        myMail.Body = myMail.Body & "in riferimento al Contratto di Noleggio in oggetto La invitiamo a prendere visione della documentazione in allegato alla presente.<br><br>"
                        myMail.Body = myMail.Body & "Cordiali saluti<br><br>"

                        'Firma                
                        myMail.Body = myMail.Body & "<a href=""www.sicilyrentcar.it""><img src=""http://ares.sicilyrentcar.it/img/logo_mail.png"" alt=""Logo SRC""></a><br>"
                        myMail.Body = myMail.Body & "Sicily Rent Car S.r.l.<br>"
                        myMail.Body = myMail.Body & "Largo Lituania, 11 <br>"
                        myMail.Body = myMail.Body & "90146 Palermo<br>"
                        myMail.Body = myMail.Body & "P.Iva 2486830819<br>"

                        myMail.Body = myMail.Body & "<p><span style='font-size:8.0pt'>Ai sensi delle vigenti disposizioni in materia si precisa che la presente e-mail, con i suoi eventuali allegati, può contenere informazioni private e/o confidenziali ed è destinata esclusivamente ai destinatari in indirizzo. Se avete ricevuto questa e-mail per errore siete espressamente diffidati dal riprodurla in tutto od in parte o, comunque,dall'utilizzare le informazioni contenute nella stessa e nei suoi eventuali allegati. Siete, altresì, pregati di voler contattare il mittente e di distruggere ogni copia di questa e-mail.</span></p><br>"

                    Else
                        myMail.Body = "Dear Customer,<br><br>"
                        myMail.Body = myMail.Body & "with reference to the Rental Agreement in question, we invite you to read the documentation attached to this document.<br><br>"
                        myMail.Body = myMail.Body & "Best regards<br><br>"
                        ...
                        myMail.Body = myMail.Body & "<span style='font-size:8.0pt'>&nbsp;</span><br><span style='font-size:8.0pt'>We inform you that this e-mail, including any attachments, may contain private and/or confidential information. If you are not the addressee or if you have received this e-mail in error, you must not use it or take any action based on this e-mail or any information herein. Please contact the sender immediately and delete any copies of this e-mail.</span></p>"
                    End If

                    mySmtp.Send(myMail)
                    Libreria.genUserMsgBox(Page, "Email Inviata")

                    'Salvataggio su RDS_mailinviate
                    MailInviata(Now, StringaEmailDestinatari, lb_id_evento.Text, ArrayAllegatiInviati)

                Catch se As SmtpException
                    Libreria.genUserMsgBox(Page, se.Message & " Errore in SMTP --- Errore contattare amministratore del sistema.")
                Catch ex As Exception
                    Libreria.genUserMsgBox(Page, ex.Message & " Errore generico mail --- Errore contattare amministratore del sistema.")
                End Try
            Else
                Libreria.genUserMsgBox(Page, "Selezionare almeno un allegato da inviare")
            End If

        Else
            Libreria.genUserMsgBox(Page, "Impossibile Inviare Mail. L'evento NON è Chiusura Contratto")
        End If
    ...
    'TONY 24-04-2023  
    Protected Sub MailInviata(ByVal data As String, ByVal Destinatario As String, ByVal IdEvento As String, ByVal AllegatiInv As String)
        Try
            Dim Dbc As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            Dbc.Open()
            ...
            Dim mmin = Date.Now.Minute
            Dim ss = Date.Now.Second
            Dim tt = yy & "-" & mm & "-" & dd & " " & hh & ":" & mmin


            Sql = "insert into RDS_mail_inviate(data,destinatario,operatore,id_evento,allegati_inviati)" & _
                  "values(CONVERT(char, '" & tt & "', 103),'" & Destinatario & "','" & Request.Cookies("SicilyRentCar")("nome") & "','" & IdEvento & "','" & AllegatiInv & "')"
            'Response.Write(Sql & "<br>")
            'Response.End()
            ...
            SqlQuery = "insert into query_log (data,utente,query) values('" & Now & "','" & Request.Cookies("SicilyRentCar")("nome") & "','" & Replace(Sql2, "'", "''") & "')"
            Cmd.CommandText = SqlQuery
            'Response.Write(Cmd.CommandText & "<br/>")
            'Response.End()
            Cmd.ExecuteNonQuery()
        ...
        Catch ex As Exception
            Libreria.genUserMsgBox(Page, ex.Message & " Setaggio Mail Inviata --- Errore contattare amministratore del sistema.")
        End Try
    End Sub
    'FINE Tony

    'TONY 24-04-2023  
    Function ControlloInvioMailRDS(ByVal IdEvento As String) As Boolean
        ...
        Try
            ...
            Dim Cmd As New Data.SqlClient.SqlCommand("select inviata_mail FROM  veicoli_evento_apertura_danno WHERE (id = '" & IdEvento & "')", Dbc)
            'Response.Write(Cmd.CommandText & "<br><br>")
            'Response.End()
            ...
            If Inviata Then
                ControlloInvioMailRDS = True
            Else
                ControlloInvioMailRDS = False
            End If
    ...
    Protected Sub bt_salva_rds_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bt_salva_rds.Click
        ...
        If Not (mio_stato_rds = sessione_danni.stato_rds.Da_lavorare Or mio_stato_rds = sessione_danni.stato_rds.In_attesa Or mio_stato_rds = sessione_danni.stato_rds.All_attenzione Or mio_stato_rds = sessione_danni.stato_rds.Da_addebitare) Then
            ...
            Try
                ...
                If Rs.HasRows Then
                    Do While Rs.Read
                        If (Rs("da_riparare") = True And Not chkDaRiparare.Checked) Or (Rs("da_riparare") = False And chkDaRiparare.Checked) Then
                            'chkDaRiparare.Checked = True
                        Else
            ...
            'If chkDaRiparare.Checked Then
            '    Libreria.genUserMsgBox(Page, "Da riparare" & vbCrLf & "1")
            'Else
            '    Libreria.genUserMsgBox(Page, "Da riparare" & vbCrLf & "0")
            'End If
            'Libreria.genUserMsgBox(Page, "Errore" & vbCrLf & mio_stato_rds & vbCrLf & "Nello stato attuale, non è possibile modificare lo stato dell'RDS.")
            'Return
        End If
        ...
        'Response.Write("Stato " & nuovo_stato_rds)        
        Select Case nuovo_stato_rds
            Case sessione_danni.stato_rds.Chiuso                
                ...
                End If
                DropDownNonAddebito.SelectedValue = 3
            ...
            Case sessione_danni.stato_rds.da_fatturare 'Stato RDS = 6 addebitato
                'Response.Write("IN Fatturare")
                'Response.End()
                ...
                Try
                    ...
                    DropOperazione.SelectedValue = nuovo_stato_rds

                    mio_evento.stato_rds = nuovo_stato_rds
                    mio_evento.AggiornaRecord()
                    mia_variazione_stato.InitDati(mio_evento, mio_stato_rds)
                    'mia_variazione_stato.SalvaRecord()
                    ...
                    Sql3 = "update veicoli_evento_apertura_danno set stato_rds=6, incasso = '" & Replace(tx_importo_totale.Text, ",", ".") & "' WHERE (id = '" & txtIdEventoApertura.Text & "') AND (attivo = 1) "
                    ...
                    SqlQuery3 = "insert into query_log (data,utente,query) values('" & Now & "','" & Request.Cookies("SicilyRentCar")("nome") & "','" & Replace(Sql3, "'", "''") & "')"
        ...
        Try
            If chkDaRiparare.Checked Then
                Sql2 = "update veicoli_evento_apertura_danno set da_riparare=1 WHERE (id_documento_apertura = '" & lb_num_documento.Text & "') AND (attivo = 1) "
            Else
                Sql2 = "update veicoli_evento_apertura_danno set da_riparare=0 WHERE (id_documento_apertura = '" & lb_num_documento.Text & "') AND (attivo = 1) "
            End If
            'Response.Write(Sql2)
            'Response.End()
            ...
            Cmd2.ExecuteNonQuery()
        Catch ex As Exception
            Libreria.genUserMsgBox(Page, "Salvataggio DA RIPARARE 2  ARES Errore contattare amministratore del sistema.")
        End Try
        ...
        'Tony 23-05-2023
        If nuovo_stato_rds = 5 And DropDownTipoEventoAperturaDanno.SelectedValue = 1 Then 'Da addebitare con RDS in RA            
            Try
                Dim SqlStato5 As String
                SqlStato5 = "select * from contratti  WITH(NOLOCK) where  num_contratto = " & lb_num_documento.Text & " and attivo=1"
                'Response.Write(SqlStato5 & "<br><br>")
                'Response.End()

                Dim DbcStato5 As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
                DbcStato5.Open()

                Dim CmdStato5 As New Data.SqlClient.SqlCommand(SqlStato5, DbcStato5)
                'Response.Write(CmdStato5.CommandText & "<br><br>")
                'Response.End()

                Dim RsStato5 As Data.SqlClient.SqlDataReader
                RsStato5 = CmdStato5.ExecuteReader()

                If RsStato5.HasRows Then
                    Do While RsStato5.Read
                        Dim DbcNonFatturare As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
                        DbcNonFatturare.Open()

                        Dim CmdNonFatturare As New Data.SqlClient.SqlCommand("", DbcNonFatturare)
                        Dim SqlNonFatturare As String
                        Dim SqlQueryNonFatturare As String

                        Try                            
                            SqlNonFatturare = "update contratti set non_fatturare=1 where  num_contratto = " & lb_num_documento.Text & " and attivo=1"
                            'Response.Write(SqlNonFatturare & "<br>")
                            'Response.End()

                            CmdNonFatturare = New Data.SqlClient.SqlCommand(SqlNonFatturare, DbcNonFatturare)
                            CmdNonFatturare.ExecuteNonQuery()

                            SqlQueryNonFatturare = "insert into query_log (data,utente,query) values('" & Now & "','" & Request.Cookies("SicilyRentCar")("nome") & "','" & Replace(SqlNonFatturare, "'", "''") & "')"
                            CmdNonFatturare.CommandText = SqlQueryNonFatturare
                            'Response.Write(CmdNonFatturare.CommandText & "<br/>")
                            'Response.End()
                            CmdNonFatturare.ExecuteNonQuery()
                        Catch ex As Exception
                            Libreria.genUserMsgBox(Page, "UPDATE NON Fatturare Errore contattare amministratore del sistema.")
                        End Try

                        CmdNonFatturare.Dispose()
                        CmdNonFatturare = Nothing
                        DbcNonFatturare.Close()
                        DbcNonFatturare.Dispose()
                        DbcNonFatturare = Nothing
                    Loop
                End If
                ...
                DbcStato5 = Nothing
            Catch ex As Exception
                Libreria.genUserMsgBox(Page, "Stato 5 Addebito ARES Errore contattare amministratore del sistema.")
            End Try
        End If
        'FINE Tony

        'Tony 22-05-2023
        If nuovo_stato_rds = 6 Then
            Dim Dbc3 As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            Dbc3.Open()
    ...
    Protected Sub bt_stampa_lettera_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bt_stampa_lettera.Click
        ...
        Session("RDS_id_evento") = lb_num_documento.Text
    ...
    'Tony 02-05-2023
    Protected Sub listViewMailInviate_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ListViewItemEventArgs) Handles listViewMailInviate.ItemDataBound
        Dim Data As Label = e.Item.FindControl("lb_data")

        Data.Text = CDate(Data.Text)
    End Sub
    'FINE Tony
End Class
