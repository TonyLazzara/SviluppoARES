...
Partial Class gestione_danni_tipo_attesa_manutenzione
    ...
    Private Sub Visibilita(Valore As DivVisibile)
        div_elenco_tipo_attesa.Visible = Valore And DivVisibile.Elenco

        div_modifica_tipo_attesa.Visible = Valore And DivVisibile.Modifica

        div_intestazione.Visible = Valore And DivVisibile.Intestazione
    End Sub

    Public Sub InitForm(Optional mio_record As veicoli_tipo_attesa = Nothing, Optional stato As Integer = 0, Optional provenienza As String = "")
        lb_stato.Text = stato
    ...
    Protected Sub FillEdit(mio_record As veicoli_tipo_attesa)
        With mio_record
            lb_id.Text = .id
    ...
    Protected Sub AzzeraEdit()
        Dim mio_record As veicoli_tipo_attesa = New veicoli_tipo_attesa
        With mio_record
            .id = 0
            .descrizione = ""
        End With
    ...
    Protected Sub listViewTipoDanni_ItemCommand(sender As Object, e As System.Web.UI.WebControls.ListViewCommandEventArgs) Handles listViewTipoDanni.ItemCommand
        ...
        ElseIf e.CommandName = "Elimina" Then
            ...
            If veicoli_tipo_attesa.CancellaRecord(Integer.Parse(id_riga.Text)) Then

                listViewTipoDanni.DataBind()

                Libreria.genUserMsgBox(Page, "Tipo attesa da ufficio manutenzione eliminato correttamente")
            Else
                Libreria.genUserMsgBox(Page, "Errore nell'eliminazione del tipo attesa da ufficio manutenzione.")
            End If
        End If
    End Sub
