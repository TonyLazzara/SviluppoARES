...
Partial Class gestione_danni_tipo_documento_danno
    ...
    Public Sub InitForm(Optional mio_record As veicoli_danni_img_tipo_documenti = Nothing, Optional stato As Integer = 0, Optional provenienza As String = "", Optional tipo_documento As tipo_danni_img_tipo_documenti = tipo_danni_img_tipo_documenti.Danno)
        lb_stato.Text = stato
        lb_provenienza.Text = provenienza
        lb_tipo_img_documento.Text = tipo_documento

        If tipo_documento = tipo_danni_img_tipo_documenti.Danno Then
            lb_intestazione.Text = "Veicoli - Gestione Tipo Documenti Per Danni"
        Else
            lb_intestazione.Text = "Veicoli - Gestione Tipo Documenti Per Evento Danni"
        End If
        Trace.Write(lb_intestazione.Text & " - " & lb_tipo_img_documento.Text)
    ...
    Protected Sub FillEdit(mio_record As veicoli_danni_img_tipo_documenti)
        With mio_record
            lb_id.Text = .id
            lb_tipo_img_documento.Text = .tipo
            tx_descrizione.Text = .descrizione
            lb_ordine.Text = .ordine
            lb_obbligatorio.Text = .obbligatorio
        End With
    End Sub

    Protected Sub AzzeraEdit()
        Dim mio_record As veicoli_danni_img_tipo_documenti = New veicoli_danni_img_tipo_documenti
        With mio_record
            .id = 0
            .tipo = Integer.Parse(lb_tipo_img_documento.Text)
            .descrizione = ""
            .ordine = 0
            .obbligatorio = False
        End With
    ...
    Protected Sub btnModifica_Click(sender As Object, e As System.EventArgs) Handles btnModifica.Click
        ...
        With mio_record
            ...
            .ordine = Integer.Parse(lb_ordine.Text)
            .obbligatorio = Boolean.Parse(lb_obbligatorio.Text)

            Trace.Write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>tipo:" & .tipo)
    ...
    Protected Sub listViewTipoDanni_ItemCommand(sender As Object, e As System.Web.UI.WebControls.ListViewCommandEventArgs) Handles listViewTipoDanni.ItemCommand
        If e.CommandName = "Modifica" Then
            ...
            Dim lb_ordine As Label = e.Item.FindControl("lb_ordine")
            Dim lb_obbligatorio As Label = e.Item.FindControl("lb_obbligatorio")
        ...
        ElseIf e.CommandName = "Elimina" Then
            ...
            If veicoli_danni_img_tipo_documenti.CancellaRecord(Integer.Parse(lb_id.Text)) Then

                listViewTipoDanni.DataBind()

                Libreria.genUserMsgBox(Page, "Tipo documento per danno veicolo eliminato correttamente")
            Else
                Libreria.genUserMsgBox(Page, "Errore nell'eliminazione del tipo documento per danno.")
            End If
        End If
    End Sub
