...
Partial Class sqltest2
    ...
    Protected Sub Elenco_Veicoli_Click(sender As Object, e As EventArgs)
        ...
        Dim ora_ini_nolo As String = "11:30"
        Dim sta_end_nolo As String = "2"
        Dim dt_end_nolo As String = "2020-10-27"
        Dim ora_end_nolo As String = "11:30"
        Dim eta As String = "26"
        Dim id_tipo_cliente As String = 3 'INT Cliente web
        Dim lingua As String = "ITA"
        Dim cod_promo As String = ""

        strElencoVeicoli = getElencoVeicoli(sta_ini_nolo, dt_ini_nolo, ora_ini_nolo, sta_end_nolo, dt_end_nolo, ora_end_nolo, eta, id_tipo_cliente, lingua, cod_promo)

        Response.Write("strElencoVeicoli: " & strElencoVeicoli & " -- " & Date.Now.ToString & "")
    ...
    Public Function getElencoVeicoli(ByVal stazione_inizio_noleggio As String, ByVal data_inizio_noleggio As String, ByVal ora_inizio_noleggio As String, ByVal stazione_fine_noleggio As String, ByVal data_fine_noleggio As String, ByVal ora_fine_noleggio As String, ByVal eta As String, ByVal id_tip_cliente As Integer, ByVal lingua As String, ByVal cod_promo As String) As String
        ...
        Try
            ...
            Dim Dbc As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            Dbc.Open()
            Dim sqla As String = "convert(date,'2020-10-26',102) "

            sqla = "SELECT stop_sell.id as id_stop_sell,* FROM stop_sell LEFT JOIN stop_sell_fonti ON stop_sell.id = stop_sell_fonti.id_stop_sell LEFT JOIN clienti_tipologia ON stop_sell_fonti.id_fonte = clienti_tipologia.id   WHERE (CONVERT(date,'" & Replace(data_inizio_completa, "/", "-") & "',102)  BETWEEN da_data AND a_data) or (CONVERT(date,'" & Replace(data_inizio_completa, "/", "-") & "',102) < da_data and CONVERT(date,'" & Replace(data_fine_completa, "/", "-") & "',102) >= da_data)"
            ' Dim Cmd As New Data.SqlClient.SqlCommand("SELECT stop_sell.id as id_stop_sell,* FROM stop_sell LEFT JOIN stop_sell_fonti ON stop_sell.id = stop_sell_fonti.id_stop_sell LEFT JOIN clienti_tipologia ON stop_sell_fonti.id_fonte = clienti_tipologia.id   WHERE ('" & data_inizio_completa & "'  BETWEEN da_data AND a_data) or ('" & data_inizio_completa & "' < da_data and '" & data_fine_completa & "' >= da_data)", Dbc)
            ...
            Else
                ...
                Do While Rs.Read()
                    ...
                    Else
                        ...
                        Dbc2.Open()
                        Dim Cmd2 As New Data.SqlClient.SqlCommand("SELECT stop_sell.id as id_stop_sell,stop_sell.*,stop_sell_stazioni.id_stazione FROM stop_sell,stop_sell_stazioni WHERE (stop_sell.id = stop_sell_stazioni.id_stop_sell) AND (('" & data_inizio_completa & "'  BETWEEN da_data AND a_data) OR ('" & data_inizio_completa & "' < da_data and '" & data_fine_completa & "' >= da_data)) AND (id_stazione = " & stazione2 & ")", Dbc2)
                ...
                Else
                    ...
                    If StazioneInStopSale = False Then
                        ...
                        If id_stop_sell <> "" Then
                            ...
                            DbcStopSale.Open()
                            Dim CmdStopSale As New Data.SqlClient.SqlCommand("SELECT stop_sell.id as id_stop_sell,* FROM stop_sell LEFT JOIN stop_sell_fonti ON stop_sell.id = stop_sell_fonti.id_stop_sell LEFT JOIN clienti_tipologia ON stop_sell_fonti.id_fonte = clienti_tipologia.id WHERE ('" & data_inizio_completa & "'  BETWEEN da_data AND a_data) or ('" & data_inizio_completa & "' < da_data and '" & data_fine_completa & "' >= da_data)", DbcStopSale)
