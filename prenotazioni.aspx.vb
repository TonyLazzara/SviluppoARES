...
Partial Class prenotazioni
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack() Then
            ...
            'If Not IsNothing(gruppoDaCalcolare.SelectedValue) Then
            '    lbl_gruppo_old.Text = gruppoDaCalcolare.SelectedValue
            'Else
            '    lbl_gruppo_old.Text = "0"
            'End If
        ...
        lbl_gruppo_old.Visible = setValueBoolean
        '@end salvo
    ...
    Protected Sub calcolaTariffe(ByVal id_tariffe_righe As String, ByVal imposta_prepagato As Boolean, Optional data_creazione As String = "", Optional ricalcola As Boolean = True)
        ...
        Dim ScontoOriginale As String = txtSconto.Text 'aggiunto salvo 31.03.2023
        ...
        'agggiunto 31.03.2023 salvo
        If Session("ricalcola_prenotazione") = "ok" Then
            txtSconto.Text = ScontoOriginale
        End If
        ...
        Dim ValoreTariffaOri As String = "0"
        If (gruppoDaCalcolare.SelectedValue = lbl_gruppo_old.Text) And (dropStazionePickUp.SelectedValue = lbl_StazionePickUp_OLD.Text) And (CDate(txtDaData.Text) = CDate(lbl_DaData_Old.Text)) And (txtoraPartenza.Text = lbl_DaOre_Old.Text) And (dropStazioneDropOff.SelectedValue = lbl_stazioneDropOff_OLD.Text) And (CDate(txtAData.Text) = CDate(lbl_AData_old.Text)) And (txtOraRientro.Text = lbl_AOre_old.Text) And (txtNumeroGiorni.Text = lbl_numeroGiorni_old.Text) Then
            ...
            SetTariffaOri = True
        End If
        'se cambia solo la stazione di dropOFF calcola solo i gg Extra presi dalla tariffa in in vigore al momento
        'del ricalcolo 07.03.2023 salvo
        Dim ggExtra As String = "0"
        Dim datediffdropoff As Integer = CInt(numero_giorni) - CInt(lbl_numeroGiorni_old.Text)  ' DateDiff("d", CDate(txtAData.Text), CDate(lbl_AData_old.Text))


        If (gruppoDaCalcolare.SelectedValue = lbl_gruppo_old.Text) And (dropStazionePickUp.SelectedValue = lbl_StazionePickUp_OLD.Text) And (CDate(txtDaData.Text) = CDate(lbl_DaData_Old.Text)) And (txtoraPartenza.Text = lbl_DaOre_Old.Text) And (dropStazioneDropOff.SelectedValue = lbl_stazioneDropOff_OLD.Text) And (datediffdropoff <> 0) And (txtOraRientro.Text = lbl_AOre_old.Text) Then
            'dati non sono cambiati recupera valore tariffa e totale del calcolo corrente
            ValoreTariffaOri = funzioni_comuni_new.GetUltimoValoreTariffaValidoPren(idPrenotazione.Text, (CInt(numCalcolo.Text) - 1))
            lbl_valore_tariffa_ori.Text = ValoreTariffaOri
            SetTariffaOri = True
            ...
            '#Aggiornato Salvo 30.05.2023
            'se ggextra <0 deve ricalcolare
            ggExtra = datediffdropoff.ToString 'giorni extra
            'If ggExtra < 0 Then
            '    ggExtra = 0
            'End If
        ...
        'Se StazionePickUp, dataPickUp, Gruppo, NigiorniNolo UGUALI lascia Tariffa Originale salvo 21.03.2023
        If (datediffdropoff = 0) And (gruppoDaCalcolare.SelectedValue = lbl_gruppo_old.Text) And (dropStazionePickUp.SelectedValue = lbl_StazionePickUp_OLD.Text) And (CDate(txtDaData.Text) = CDate(lbl_DaData_Old.Text)) Then
            ValoreTariffaOri = funzioni_comuni_new.GetUltimoValoreTariffaValidoPren(idPrenotazione.Text, (CInt(numCalcolo.Text) - 1))
            lbl_valore_tariffa_ori.Text = ValoreTariffaOri
            SetTariffaOri = True
        End If

        '# Salvo  aggiornato 06.06.2023
        'se giorni extra minori per riduzione periodo ricalcola con nuova tariffa
        If ggExtra < 0 And lblPrepagato.Visible = False Then
            SetTariffaOri = False
            lbl_valore_tariffa_ori.Text = "0"
            ValoreTariffaOri = "0"
        End If
        'si tratta di prepagato deve mantenere tutto inalterato se ggextra <0 altrimenti calcola il ggextra
        'ggextra=0 e tariffaOriginale true
        If lblPrepagato.Visible = True Then
            ValoreTariffaOri = funzioni_comuni_new.GetUltimoValoreTariffaValidoPren(idPrenotazione.Text, (CInt(numCalcolo.Text) - 1))
            lbl_valore_tariffa_ori.Text = ValoreTariffaOri
            SetTariffaOri = True
            ggExtra = datediffdropoff.ToString 'giorni extra
            'If ggExtra < 0 Then
            If ggExtra < 0 Then    'aggiunto salvo 16.06.2023 
                ggExtra = 0
            End If
        End If
        '@end salvo aggiornato 06.06.2023


        '# salvo 12.07.2023 se cambia gruppo deve sempre ricalcolare con tariffa corrente salvo 12.07.2023
        If (gruppoDaCalcolare.SelectedValue <> lbl_gruppo_old.Text) Then
            SetTariffaOri = False
            lbl_valore_tariffa_ori.Text = "0"
            ValoreTariffaOri = "0"
            Session("cambio_gruppo_prenotazione") = "OK"  'aggiunto salvo 23.07.2023
        Else
            Session("cambio_gruppo_prenotazione") = ""  'aggiunto salvo 23.07.2023
        End If
        '@ end salvo
                ...
                If sel_gruppo.Checked Then
                    ...
                    If imposta_prepagato OrElse ((Not txtGiorniPrepagati.Visible) AndAlso ((tariffaVendibile) OrElse (tariffa_broker.Text = "1" And CInt(txtNumeroGiorniTO.Text) > CInt(txtNumeroGiorni.Text)))) Then
                        ...
                        funzioni.calcolaTariffa_x_gruppo(dropStazionePickUp.SelectedValue, txtDaData.Text, CInt(ore1.Text), CInt(minuti1.Text), dropStazioneDropOff.SelectedValue, id_tariffe_righe, id_gruppo.Text, "",
                                                         ...
                                                         txtSconto.Text,, ggExtra, ValoreTariffaOri, SetTariffaOri)
                    Else
    ...
    Protected Sub richiama_prenotazione(ByVal num_pren As String)
        ...
        '# salvo aggiunto 05.07.2023
        'ora di inizio e fine in cui una prenotazione scaduta il giorno prima
        'perchè aperta dopo le ore 00 del giorno dopo deve risultare attiva comunque
        Dim startOraExtra As Integer = 0
        Dim endOraExtra As Integer = 3
        '@ end salvo
        ...
        Try
            ...
            If Rs.HasRows() Then
                ...
                End If
                lbl_gruppo_old.Text = Rs!id_gruppo   'salvo 10.03.2023
                ...
                If (Rs("id_motivo_annullamento") & "") <> "" Then
                    ...
                    '# aggiornamento salvo 05.07.2023
                    'se ora corrente è dalle 00:00 alle 03:00 
                    'nn considera prenotazione scaduta anche se del giorno prima
                    Dim DifferenzaGG As Integer = DateDiff(DateInterval.Day, oggi, uscita)
                    'se del giorno prima la considera scaduta
                    'a meno che l'ora corrente sia dalle 00:00 alle 03:00 del giorno dopo
                    If DifferenzaGG < 0 Then
                        If DifferenzaGG = -1 Then
                            'la prenotazione risulta del giorno prima 
                            'perchè l'ora corrente di apertura della prenotazione è tra
                            'le ore 00:00-03:00. In questo caso lascia attiva la prenotazione
                            If Hour(Now) >= startOraExtra And Hour(Now) <= endOraExtra Then
                    ...
                    Dim DifferenzaGG As Integer = DateDiff(DateInterval.Day, oggi, uscita) 'aggiunto salvo 05.07.2023
                    If DifferenzaGG = 0 And dropStazionePickUp.SelectedValue = Request.Cookies("SicilyRentCar")("stazione") Then
                        'UNA PRENOTAZIONE ATTIVA PUO' DIVENTARE CONTRATTO SOLO SE E' DEL GIORNO ODIERNO
                        btnContratto.Visible = True
                    End If

                    '#aggiunto salvo 05.07.2023 solo se operatore della stessa stazione di pickup
                    'rende visibile pulsante Contratto
                    If DifferenzaGG = -1 And dropStazionePickUp.SelectedValue = Request.Cookies("SicilyRentCar")("stazione") Then
                        'la prenotazione risulta del giorno prima 
                        'perchè l'ora corrente di apertura della prenotazione è tra
                        'le ore 00:00-03:00 (impostate ad inizio procedura)
                        'In questo caso lascia attiva la prenotazione
                        'e il pulsante Contratto risulterà visibile 
                        If Hour(Now) >= startOraExtra And Hour(Now) <= endOraExtra Then
                            btnContratto.Visible = True
                        End If
                    End If
    ...
    Protected Sub salvaPrenotazione(ByVal imposta_prepagato As Boolean)
        ...
        Try
            ...
            If Rs.Read() Then
                ...
                Dim ris As Integer = Cmd.ExecuteNonQuery()
                '# Aggiunto Salvo 10.03.2023 
                If ris > 0 Then
                    lbl_gruppo_old.Text = dropGruppoDaConsegnare.SelectedValue
                End If
                '@end salvo
    ...
    Protected Sub annulla_prenotazione()
        ...
        'Tony 17/03/2023
        If lblPrepagato.Visible Then
            InvioMail(lblNumPrenotazione.Text)
        End If        
        'FINE Tony
    ...
    Protected Sub btnRicalcola_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRicalcola.Click
        ...
        'Ricalcola
        Session("ricalcola_prenotazione") = "ok"
        ricalcola_non_broker(False)
        ...
        'aggiunto salvo 31.03.2023
        'svuota session
        Session("ricalcola_prenotazione") = ""
    ...
    Protected Sub salva_contratto()
        ...
        Try
            ...
            'aggiornato 08.03.2023 salvo - anche se ora di uscita > dell'ora corrente
            If Hour(Date.Now) < CInt(ore1.Text) Or Hour(Date.Now) > CInt(ore1.Text) Then    '04.03.21 e 08.03.2023
            ...
            Dim ris() As String = funzioni_comuni_new.getVerificaValoriList("prenotazioni", idPrenotazione.Text, "0")
            ...
            '#aggiunto x verificare se fuori orario in uscita salvo 04.07.2023 
            'VALORI RESTITUITI
            '0) STAZIONE CHIUSA
            ...
            'Response.Write("ris: " & funzioni_comuni.stazione_aperta_pick_up(dropStazionePickUp.SelectedValue, txtDaData.Text, ore1.Text, minuti1.Text, "", "", idPrenotazione.Text, "", numCalcolo.Text, Request.Cookies("SicilyRentCar")("idUtente")))
            'Response.End()
            If funzioni_comuni.stazione_aperta_pick_up(dropStazionePickUp.SelectedValue, txtDaData.Text, ore1.Text, minuti1.Text, "", "", idPrenotazione.Text, "", numCalcolo.Text, Request.Cookies("SicilyRentCar")("idUtente")) = 3 Then
                'Response.Write("inizio<br/>")

                Dim iv() As String = funzioni_comuni_new.get_elemento_fuori_orario_pickUp_new(CInt(ore1.Text), CInt(minuti1.Text))
                'se iv è 168, 169 o 222 è presente un fuori orario

                'Response.Write("iv=" & iv(0) & "<br/>")

                If iv(0) <> "0" Then 'si tratta di un fuori orario della stazione al passaggio da pren a contratto

                    'verifica se già presente in contratti costi
                    sqla2 = "select id_elemento from contratti_costi where id_documento='" & id_contratto & "' and id_elemento='" & iv(0) & "'"
                    Cmd = New Data.SqlClient.SqlCommand(sqla2, Dbc)
                    Dim r2 As Integer = Cmd.ExecuteScalar()
                    If r2 = 0 Then ' se nessun valore trovato inserisce riga supplemento in contratti_costi

                        Dim idcondizione As String = funzioni_comuni_new.GetIdCondizione(id_tariffe_righe)

                        'Response.Write("idcondizione=" & idcondizione & "<br/>")

                        Dim costoFO As String = funzioni_comuni_new.GetCostoFO(idcondizione, iv(0))

                        Dim idelement As String = iv(0)

                        'verifica se già presente qualche supplemento su contratti costi - aggiunto salvo 05.07.2023
                        sqla2 = "select id_elemento from contratti_costi where id_documento='" & id_contratto & "' AND num_calcolo=0 AND id_elemento=168"
                        Dim Rs2 As Data.SqlClient.SqlDataReader
                        Cmd = New Data.SqlClient.SqlCommand(sqla2, Dbc)
                        Rs2 = Cmd.ExecuteReader()
                        If Rs2.HasRows Then
                            Rs2.Read()
                            Dim nuovoCostoDiff As String = Rs2!valore_costo
                            If CDbl(nuovoCostoDiff) > CDbl(costoFO) Then
                                'è già presente il supplemento da 50
                                'deve aggiungere solo la differenza extra per il supplemento dopo le 24
                                costoFO = CDbl(nuovoCostoDiff) > CDbl(nuovoCostoDiff)
                            End If
                            idelement = "222"    'quello del supplemento fo dopo le 24 in presenza dell'altro costo di 50€ aggiunto
                        Else
                            'non è previsto alcun supplemento deve aggiungere l'intera cifra
                        End If
                        Rs2.Close()

                        If CDbl(costoFO) > 0 Then

                            Dim imponibile As String = CDbl(costoFO) / 1.22
                            Dim iva_imponibile As String = CDbl(costoFO) - CDbl(imponibile)

                            imponibile = imponibile.Replace(".", "")
                            imponibile = imponibile.Replace(",", ".")

                            iva_imponibile = iva_imponibile.Replace(".", "")
                            iva_imponibile = iva_imponibile.Replace(",", ".")

                            costoFO = costoFO.Replace(".", "")
                            costoFO = costoFO.Replace(",", ".")

                            Dim t As String = "Ritiro fuori orario entro le 24:00"
                            If iv(0) = "169" Then
                                t = iv(1)
                            End If

                            If CInt(ore1.Text) >= 0 And CInt(ore1.Text) <= 3 Then
                                t = "Supplemento fuori orario dopo le 24:00"
                            End If


                            sqla2 = "insert into contratti_costi ("
                            sqla2 += "id_documento, id_elemento, nome_costo, selezionato, ordine_stampa, num_calcolo, id_gruppo, prepagato, valore_costo "
                            sqla2 += ",imponibile,iva_imponibile,imponibile_scontato,iva_imponibile_scontato,aliquota_iva,iva_inclusa"
                            sqla2 += ",scontabile,omaggiabile,acquistabile_nolo_in_corso,id_a_carico_di,id_metodo_stampa,obbligatorio,id_unita_misura"
                            sqla2 += ",qta,packed"
                            sqla2 += ")values("
                            sqla2 += "'" & id_contratto & "','" & idelement & "','" & t & "','1','3','0','" & id_gruppo_auto_scelto.Text & "','0','" & costoFO & "'"
                            sqla2 += ",'" & imponibile & "','" & iva_imponibile & "','" & imponibile & "','" & iva_imponibile & "','22','1'"
                            sqla2 += ",'0','0','0','2','2','1','0'"
                            sqla2 += ",'1','0'"
                            ...
                            'Response.Write("INSERT sqla2=" & sqla2 & "<br/>")
                            ...
                            'aggiorna totale
                            sqla2 = "UPDATE contratti_costi set valore_costo=valore_costo+" & costoFO & ","
                            sqla2 += "imponibile=(valore_costo+" & costoFO & ")/1.22, iva_imponibile = (valore_costo+" & costoFO & ") - ((valore_costo+" & costoFO & ")/1.22) "
                            sqla2 += ",imponibile_scontato=(valore_costo+" & costoFO & ")/1.22, iva_imponibile_scontato = (valore_costo+" & costoFO & ") - ((valore_costo+" & costoFO & ")/1.22) "
                            sqla2 += "where nome_costo ='TOTALE' and id_documento='" & id_contratto & "'"

                            'Response.Write("UPDATE sqla2=" & sqla2 & "<br/>")

                            Cmd = New Data.SqlClient.SqlCommand(sqla2, Dbc)
                            Dim uu As Integer = Cmd.ExecuteNonQuery()
                ...
                End If
            End If
            '@end salvo verifica fuori orario
        ...
        Catch ex As Exception
            HttpContext.Current.Response.Write("error prenotazioni salva_contratto  : <br/>" & ex.Message & "<br/>" & sqla & "<br/>" & sqla2 & "<br/>")
    ...
    Protected Sub inviaMailPrenotazione(ByVal mail_conducente As String)
        ...
        "<b>Conducente / Driver: </b>" & txtCognomeConducente.Text & " " & txtNomeConducente.Text & "<br>"
            ...
            corpoMessaggio = corpoMessaggio & "<b>Categoria auto / Car group: </b> " & Cmd.ExecuteScalar & " o similare<br><br><b>RITIRO / PICK-UP </b><br>" &
            ...
            If (Rs("telefono") & "") <> "" Then
                corpoMessaggio = corpoMessaggio & " Telefono " & Rs("telefono") & "<br>"
            End If

            corpoMessaggio = corpoMessaggio & txtDaData.Text & " ore " & txtoraPartenza.Text
            ...
            corpoMessaggio = corpoMessaggio & txtAData.Text & " ore " & txtOraRientro.Text
    ...
    Protected Sub btnStampaTKm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnStampaTKm.Click
        ...
        Else
            ...
            '#Salvo 27.06.2023 aggiunto
            'se si tratta di nuovo metodo di visualizzazione
            ...
            If lblNumPrenotazione.Text <> "" Then  'aggiunto 27.06.2023 salvo
                'verifica se si tratta di nuovo/old sistema di recupero salvo 27.06.2023
                'cerca il valore nel campo list_tariffe
                If funzioni_comuni_new.VerificaListTariffePDF("prenotazioni", idPrenotazione.Text) = True Then
                    newPDF = "1"
            ...
            'altrimenti recupera con il vecchio metodo
            'aggiunto 27.06.2023
            If newPDF = "0" Then
            ...
            If tariffa_broker Then
                If funzioni_comuni.getLivelloAccesso(Request.Cookies("SicilyRentCar")("idUtente"), PermessiUtente.VisualizzareValoreTariffaBroker) <> "1" Then
                    ...
                    If (Not ClientScript.IsClientScriptBlockRegistered("NewWindow")) Then
                        Session("url_print") = "/stampe/" & fileStampa & "?pagina=verticale&id_tempo_km=" & id_tempo_km & "&nro=" & lblNumPrenotazione.Text & "&iddoc=" & idPrenotazione.Text & "&tbl=prenotazioni"
                        If (Not ClientScript.IsClientScriptBlockRegistered("NewWindow")) Then
    ...
    'Tony 17/03/2023
    Private Sub InvioMail(ByVal NumPrenotazione As String)
        Try
            ...
            myMail.Subject = "Prenotazione " & NumPrenotazione & " Annullata "
            'myMail.Attachments.Add(New Attachment(allegato))
            myMail.IsBodyHtml = True
            myMail.Body = "In data Odierna la prenotazione Num: " & NumPrenotazione & " è stata annullata da: " & Request.Cookies("SicilyRentCar")("idUtente") & "<br>"
            mySmtp.Send(myMail)
            Console.WriteLine("Email inviata")

            ScriviSuFileOutPut("In per oggi " & Now & "con ID: " & Request.Cookies("SicilyRentCar")("idUtente"))
        Catch e As Exception
            Console.WriteLine(e.ToString)
            ScriviSuFileOutPutErrore(e.ToString)
        End Try
    End Sub

    Private Sub ScriviSuFileOutPutErrore(ByVal Istruzione As String)
        'Dim fs As FileStream = File.Create("OutPut.ini")
        'fs.Close()

        Dim FileInScrittura As System.IO.StreamWriter
        FileInScrittura = My.Computer.FileSystem.OpenTextFileWriter("Errore.ini", True)
        FileInScrittura.WriteLine(Now & " " & Istruzione & ";")
        FileInScrittura.Close()
    End Sub

    Private Sub ScriviSuFileOutPut(ByVal Istruzione As String)
        'Dim fs As FileStream = File.Create("OutPut.ini")
        'fs.Close()

        Dim FileInScrittura As System.IO.StreamWriter
        FileInScrittura = My.Computer.FileSystem.OpenTextFileWriter("OutPut.ini", True)
        FileInScrittura.WriteLine(Now & " " & Istruzione & ";")
        FileInScrittura.Close()
