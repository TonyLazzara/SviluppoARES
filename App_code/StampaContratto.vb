...
Imports iTextSharp.text.pdf       'aggiunto  09.12.2021 1750
...
Public Class DatiStampaContratto
    ...
    Public data_uscita As String  'aggiunto salvo 12.04.2023
    Public data_rientro As String  'aggiunto salvo 12.04.2023
    ...
    Public Shared Function GeneraDocumento(mie_dati As DatiStampaContratto, Optional ByVal nazione As String = "", Optional ByVal id_nazione As String = "", Optional tipostampa As String = "uscita") As MemoryStream
        ...
        Try
            ...
            With mie_dati
                ...
                'aggiunto salvo 12.04.2023 e modifcato il 27.07.2023 salvo inizio nome campo minuscolo
                If tipostampa = "rientro" Then
                    pdfFormFields.SetField("data_firma", .data_rientro) 'aggiunto x data salvo 12.04.2023   si tratta di uscita
                Else
                    pdfFormFields.SetField("data_firma", .data_uscita) 'aggiunto x data salvo 12.04.2023   si tratta di uscita
                End If
                ...
                If .firma Or statusRA = "8" Or statusRA = "4" Or statusRA = "6" Then   'aggiunto status 6 per stampa x recupero 17.01.2023
                    ...
                    If filefirmaPresente = True Then        'se presente firma la inserisce 06.07.2022 salvo - genera_documento
                        ...
                        If nazione = "ITALIA" Or nazione = "16" Then
                            ...
                            PdfStamper.GetOverContent(1).AddImage(img_firma, 160, 0, 0, 80, 370, 130) '1^firma 1^pag
                            PdfStamper.GetOverContent(1).AddImage(img_firma, 80, 0, 0, 60, 390, 5)   '2^firma  1^pag 
                            PdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 180, 95)   '1^firma ultima pagina AddImage(img_firma, 80, 0, 0, 50, margine sx, margine inferiore)
                            PdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 180, 5)   '2^firma ultima pagina 'old PdfStamper.GetOverContent(2).AddImage(img_firma, 80, 0, 0, 50, 420, -5)
                        Else
                            'ALTRE NAZIONI ENG
                            Dim img_firma As Image
                            img_firma = Image.GetInstance(New Uri(HttpContext.Current.Request.MapPath(HttpContext.Current.Request.ApplicationPath) & "\firme_contratti\pick_up\" & firma_def))
                            PdfStamper.GetOverContent(1).AddImage(img_firma, 160, 0, 0, 80, 390, 140) '1^firma 1^pag
                            PdfStamper.GetOverContent(1).AddImage(img_firma, 80, 0, 0, 60, 397, 18)   '2^firma  1^pag
                            PdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 196, 425)   '1^firma ultima pagina AddImage(img_firma, 80, 0, 0, 50, margine sx, margine inferiore)
                            PdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 196, 325)   '2^firma ultima pagina 
                            'old
                            'PdfStamper.GetOverContent(2).AddImage(img_firma, 80, 0, 0, 50, 420, -5)
                        End If
                        '# SALVO nota x posizione firme: nel margine inferiore diminuendo il valore la firma si posiziona sempre più in basso - 21.06.2023
                    Else
                            ...
                            If Flag_filefirmaPresente = True Then  'inserito 06.07.2022 salvo genera_documento_PDF
                                ...
                                If nazione = "ITALIA" Or nazione = "16" Then
                                    If flag_insert_firma = True Then    'se flag firma true
                                        ...
                                        myPdfStamper.GetOverContent(1).AddImage(img_firma, 160, 0, 0, 80, 370, 130) '1^firma 1^pag
                                        myPdfStamper.GetOverContent(1).AddImage(img_firma, 80, 0, 0, 60, 390, 5)   '2^firma  1^pag 
                                        myPdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 180, 95)   '1^firma ultima pagina AddImage(img_firma, 80, 0, 0, 50, margine sx, margine inferiore)
                                        myPdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 180, 5)   '2^firma ultima pagina 'old PdfStamper.GetOverContent(2).AddImage(img_firma, 80, 0, 0, 50, 420, -5)
                                ...
                                Else
                                    If flag_insert_firma = True Then    'se flag firma true
                                        ...
                                        myPdfStamper.GetOverContent(1).AddImage(img_firma, 160, 0, 0, 80, 390, 140) '1^firma 1^pag
                                        myPdfStamper.GetOverContent(1).AddImage(img_firma, 80, 0, 0, 60, 397, 18)   '2^firma  1^pag
                                        myPdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 196, 425)  '1^firma ultima pagina AddImage(img_firma, 80, 0, 0, 50, margine sx, margine inferiore)
                                        myPdfStamper.GetOverContent(5).AddImage(img_firma, 80, 0, 0, 80, 196, 325)   '2^firma ultima pagina 
                                        'old
                                        'PdfStamper.GetOverContent(2).AddImage(img_firma, 80, 0, 0, 50, 420, -5)
                                        'Nota: riaggiornato salvo il 27.07.2023
