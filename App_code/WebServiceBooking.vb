'22.06.2021 17:59 da server 234 
'GetElencoVeicoli
...
<WebService(Namespace:="http://sviluppoares.sicilyrentcar.it/")>
<WebServiceBinding(ConformsTo:=WsiProfiles.BasicProfile1_1)> _
<Global.Microsoft.VisualBasic.CompilerServices.DesignerGenerated()> _
Public Class WebServiceBooking
    ...
    Function get_tariffa_x_righe(ByVal data_uscita As String, ByVal data_rientro As String, ByVal giorni_nolo As String, ByVal stazione As String, ByVal stazioneOff As String, ByVal IdTipCliente As Integer, ByVal cod_promo As String) As String
        ...
        Dim pickup_da As String = pickup_da_temp.Year & "-" & pickup_da_temp.Month & "-" & pickup_da_temp.Day & " 00:00:00"
        Dim pickup_a As String = pickup_a_temp.Year & "-" & pickup_a_temp.Month & "-" & pickup_a_temp.Day & " 23:59:59"
        ...
        Try
            If cod_promo & "" <> "" Then
                ...
                sql = "SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa WHERE  "
                sql += "tariffe.attiva='1' AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a  AND convert(datetime,'" & pickup_da & "',102) "
                sql += "BETWEEN tariffe_righe.pickup_da And tariffe_righe.pickup_a And tariffe.codicepromozionale='" & cod_promo & "' ORDER BY codice"
            ...
            Else
                ...
                sql = "(SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa "
                sql += "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '0') AND tariffe.attiva='1' " & condizione_id_prev & " AND convert(datetime,'" & pickup_da & "',102) BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a "
                sql += "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a "
                sql += "And tariffe.id Not In (Select DISTINCT id_tariffa FROM tariffe_X_stazioni With(NOLOCK) WHERE id_tariffa=tariffe.id) "
                sql += "AND tariffe.id NOT IN (SELECT DISTINCT id_tariffa FROM tariffe_X_fonti WITH(NOLOCK) WHERE id_tariffa=tariffe.id) "
                sql += "AND convert(datetime,'" & pickup_a & "',102) <= ISNULL(max_data_rientro, '3000-12-12 23:59:59')) ORDER BY codice"
            ...
            "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '0') AND tariffe.attiva='1' " & condizione_id_prev & " AND convert(datetime,'" & pickup_da & "',102) BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " &
            "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a " &
            "AND convert(datetime,'" & pickup_a & "',102) <= ISNULL(max_data_rientro, '3000-12-12 23:59:59') " &
            "AND ( (" &
        ...
        Catch ex As Exception
            get_tariffa_x_righe = "getrighe:" & sql
    ...
    Function get_tariffa_x_righe_prepagato(ByVal data_uscita As String, ByVal data_rientro As String, ByVal giorni_nolo As String, ByVal stazione As String, ByVal stazioneOff As String, ByVal IdTipCliente As Integer, ByVal cod_promo As String) As String
        ...
        Dim pickup_da As String = pickup_da_temp.Year & "-" & pickup_da_temp.Month & "-" & pickup_da_temp.Day & " 00:00:00" 'modificato il 20.06.2021
        Dim pickup_a As String = pickup_a_temp.Year & "-" & pickup_a_temp.Month & "-" & pickup_a_temp.Day & " 23:59:59"

        If cod_promo & "" <> "" Then
            'Dim Sql = "SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa WHERE  tariffe.attiva='1' AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a  AND  tariffe.codicepromozionale='" & cod_promo & "' ORDER BY codice"
            Dim Sql = "SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa "
            Sql += "WHERE  tariffe.attiva='1' AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a  "
            Sql += " And convert(datetime,'" & pickup_da & "',102) BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a AND  tariffe.codicepromozionale='" & cod_promo & "' ORDER BY codice"
        ...
        Else
           ...
           "INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa " &
           "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '1') AND tariffe.attiva='1' " & condizione_id_prev & " AND convert(datetime,'" & pickup_da & "',102) BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " &
           "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a " &
           ...
           "AND convert(datetime,'" & pickup_a & "',102) <= ISNULL(max_data_rientro, '3000-12-12 23:59:59')) ORDER BY codice"
    ...
    Function calcolaTariffa_x_gruppo(ByVal stazione_pick_up As String, ByVal data_pick_up As String, ByVal ore_pick_up As Integer,
                                       ...
                                       Optional idTariffaBanco As String = "", Optional idTariffaOnLine As String = "") As String 'aggiunta per webservice


        'inserita ultima riga Optional x calcolo nuova tariffa periodi - salvo 06.12.2022 - 04.01.2023

        Dim ris As String = "ok"
        ...
        ris = calcola_tempo_km(giorni_noleggio, giorni_prepagati_x_modifica, prepagata, giorni_noleggio_extra_rack, sconto_su_rack, stazione_pick_up,
                         ...
                         data_pick_up, data_drop_off, tipoCli, tipoTariffa, descTariffa, idTariffa, data_creazione, max_sconto_new, idTariffaBanco, idTariffaOnLine)
        ...
        If stazione_pick_up <> stazione_drop_off Then
            If gps_obbligatorio_o_incluso(id_gruppo, id_preventivo, id_ribaltamento, id_prenotazione, id_contratto, num_calcolo) Then
                aggiungi_val_gps(stazione_pick_up, stazione_drop_off, id_gruppo, giorni_noleggio, giorni_prepagati_x_modifica, prepagata, elementi_prepagati, "", "", sconto,
                                 id_tariffe_righe, id_preventivo, id_ribaltamento, id_prenotazione, id_contratto, num_calcolo, id_utente, tipo_commissione, commissione_percentuale, id_fonte_commissionabile)
            End If
        End If
        ...
        If prepagata And giorni_prepagati_x_modifica > 0 Then
            ...
            'PRECEDENTE
            aggiungi_accessori_obbligatori_prepagati_calcolo_precedente(id_prenotazione, id_contratto, num_calcolo, id_gruppo, stazione_pick_up, stazione_drop_off,
                                                                        giorni_noleggio, giorni_prepagati_x_modifica, sconto, id_tariffe_righe, id_utente)
        ElseIf prepagata And giorni_prepagati_x_modifica = 0 Then
            ...
            'prepagato_memorizza_costi_prepagati_x_fattura(id_prenotazione, id_ribaltamento, id_contratto, id_gruppo, num_calcolo, "")                      'questo su funzioni
            prepagato_memorizza_costi_prepagati_x_fattura(id_preventivo, id_prenotazione, id_ribaltamento, id_contratto, id_gruppo, num_calcolo, "")        'questo su la precedente procedura
        End If
        ...
        Return ris & "- CTG: " & descTariffa & "- idTariffa:" & idTariffa & "-"
    ...
    Function calcolaTariffa_x_gruppo_NEW(ByVal stazione_pick_up As String, ByVal data_pick_up As String, ByVal ore_pick_up As Integer,
                                       ByVal minuti_pick_up As Integer, ByVal stazione_drop_off As String, ByVal id_tariffe_righe As String,
                                       ByVal id_gruppo As String, ByVal id_gruppo_da_prenotazione_x_modifica_con_rack As String,
                                       ByVal giorni_noleggio As Integer, ByVal giorni_prepagati_x_modifica As String, ByVal prepagata As String,
                                       ByVal giorni_noleggio_extra_rack As Integer, ByVal sconto As Double, ByVal tipo_sconto As String,
    ...
    'Inserita 04/06/2015
    Sub calcolaTariffa_x_gruppo_EX(ByVal stazione_pick_up As String, ByVal data_pick_up As String,
                                ByVal ore_pick_up As Integer, ByVal minuti_pick_up As Integer,
                                ByVal stazione_drop_off As String, ByVal id_tariffe_righe As String,
                                ByVal id_gruppo As String,
                                ByVal id_gruppo_da_prenotazione_x_modifica_con_rack As String,
                                ByVal giorni_noleggio As Integer, ByVal giorni_prepagati_x_modifica As String,
                                ByVal prepagata As String, ByVal giorni_noleggio_extra_rack As Integer,
                                ByVal sconto As Double, ByVal tipo_sconto As String,
                                ByVal sconto_web_prepagato_primo_calcolo As Double,
                                ByVal sconto_su_rack As Double, ByVal eta_primo_guidatore As String,
                                ByVal eta_secondo_guidatore As String, ByVal id_preventivo As String,
                                ByVal id_ribaltamento As String, ByVal id_prenotazione As String,
                                ByVal id_contratto As String, ByVal num_calcolo As String,
                                ByVal id_utente As String, ByVal id_ditta As String,
                                ByVal id_fonte_commissionabile As String,
                                ByVal commissione_percentuale As String, ByVal tipo_commissione As String,
                                ByVal primo_calcolo_commissione As Boolean,
                                ByVal giorni_commissioni_originale As String,
                                Optional tipoTariffa As String = "0", Optional descTariffa As String = "0", Optional data_drop_off As String = "0",
                    Optional tipoCli As String = "0", Optional data_creazione As String = "", Optional max_sconto_new As String = "0")


        'aggiungere i paramentri mancanti salvo 01.02.2023 test
        ...
        '    'PASSO 3: RECUPERO IL COSTO DELLA TARIFFA DAL TEMPO-KM
        '    calcola_tempo_km(giorni_noleggio, giorni_prepagati_x_modifica, prepagata, giorni_noleggio_extra_rack,
        '                     sconto_su_rack, stazione_pick_up, stazione_drop_off, id_tempo_km_figlia,
        '                     id_tempo_km_rack, id_gruppo, id_gruppo_da_prenotazione_x_modifica_con_rack,
        '                     id_preventivo, id_ribaltamento, id_prenotazione, id_contratto, num_calcolo)
    ...
    Function calcola_tempo_km(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean,
                                   ...
                                   Optional IdTariffaBanco As String = "", Optional idTariffaOnLine As String = "") As String 'aggiunto x webservice
        ...
        Dim ris As String = "calcolotempokmok"
                ...
                If valore <> 0 Then
                        ...
                        If Rs.Read() Then
                            ...
                            If Not Rs("pac") Then
                                ...
                                If FlagOldCalcolo = False Then
                                    ...
                                    tipoCli, id_gruppo, TipoTariffa, descTariffa, giorni_noleggio, idTariffa, False, "", "", max_sconto_new.ToString, False) 'nuovo calcolo senza periodi 06.12.2022
                                Else
                    ...
                    Else
                        ...
                        If packed = "False" Then
                            ...
                            If FlagOldCalcolo = False Then
                                valore = funzioni_comuni_new.GetNewTariffaTempoKmPeriodi(stazione_pick_up, stazione_drop_off, dataPickUp, dataDropOff,
                                                                                     tipoCli, id_gruppo, TipoTariffa, descTariffa, giorni_noleggio, idTariffa, False, "", "", max_sconto_new, False) 'nuovo calcolo senza periodi 06.12.2022
                            Else
                            ...
                            Dim p As String = "(" & stazione_pick_up & "," & stazione_drop_off & "," & dataPickUp & "," & dataDropOff & "," & tipoCli & "," & id_gruppo & "," & TipoTariffa & "," & descTariffa & "," & giorni_noleggio & "," & idTariffa & ",false," & "" & "," & "" & "," & max_sconto_new & ")"

                            p = valore.ToString '& " " & p 'x test

                            new_Valore = valore 'x test
        ...
        Catch ex As Exception
            ...
            ris = "calcolotempokm error:" & ex.Message

        End Try

        Dim parametri As String = stazione_pick_up & "-" & stazione_drop_off & "-" & dataPickUp & "-" & dataDropOff & "-" & tipoCli & "-" & id_gruppo & "-" & TipoTariffa & "-" & descTariffa & "-" & giorni_noleggio & "-" & idTariffa & "-" & "False" & "-" & "vuoto" & "-" & "vuoto" & "-" & max_sconto_new.ToString

        Return ris '& "valore: " & new_Valore.ToString & "-" & errorLine '& "-" & parametri


    End Function


    Function calcola_tempo_km_new(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean,
                                   ByVal giorni_noleggio_extra_rack As Integer, ByVal sconto_su_rack As String, ByVal stazione_pick_up As String,
                                   ByVal stazione_drop_off As String, ByVal id_tempo_km_figlia As String, ByVal id_tempo_km_rack As String,
                                   ByVal id_gruppo As String, ByVal id_gruppo_da_prenotazione_x_modifica_con_rack As String, ByVal id_preventivo As String,
                                   ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String,
    ...
    'Inserita 04/06/2015
    Sub calcola_tempo_km_EX(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean, ByVal giorni_noleggio_extra_rack As Integer, ByVal sconto_su_rack As String,
                            ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal id_tempo_km_figlia As String, ByVal id_tempo_km_rack As String, ByVal id_gruppo As String,
                            ByVal id_gruppo_da_prenotazione_x_modifica_con_rack As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String,
                            ByVal id_contratto As String, ByVal num_calcolo As String)
    ...
    Protected Sub analisi_condizioni(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean, ByVal elementi_prepagati As Collection, ByVal data_pick_up As String, ByVal ore_pick_up As Integer, ByVal minuti_pick_up As Integer, ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal id_condizione_figlia As String, ByVal id_condizione_madre As String, ByVal id_gruppo As String, ByVal id_ditta As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String)
        ...
        Try
            ...
            '# AGGIORNAMENTO per sostituzione procedura copiata da funzioni_comuni salvo 13.10.2022 salvo
            '-->la riga seguente è quella originale in funzioni_comuni .. 13.10.2022 salvo
            'Dim fuori_orario_pick_up As String = stazione_aperta_pick_up(stazione_pick_up, data_pick_up, ore_pick_up, minuti_pick_up, "", "", "", "", "", "")
            '.. --> ed è stata sostituita da quella seguente che è presente nella stessa procedura nel webservice 13.10.2022 salvo
            '.. --> e che contiene meno parametri 
            Dim fuori_orario_pick_up As String = stazione_aperta_pick_up(stazione_pick_up, data_pick_up, ore_pick_up, minuti_pick_up)
            '@ END AGGIORNAMENTO
            ...
            Do While Rs.Read()
                ...
                ElseIf Rs("id_unita_misura") = Costanti.id_unita_misura_giorni Then
                    ...
                    ElseIf Rs("applicabilita_da") > 1 Then 'righe successive se presenti sullo stesso elemento 
                        ...
                        If idele = Rs!id_elemento Then
                            If giorni_nolo >= Rs!applicabilita_a Then
                                importo += Rs!costo * ((Rs!applicabilita_a - Rs!applicabilita_da) + 1) 'costo per il range di giorni di questo record 01.10.2022
                            Else
                                If giorni_nolo <= Rs!applicabilita_da Then
                                    importo += Rs!costo * ((giorni_nolo - Rs!applicabilita_da) + 1)
    ...
    'Inserita 04/06/2015 - modificata con nuova il 13.10.2022
    Sub analisi_condizioni_old(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean, ByVal elementi_prepagati As Collection, ByVal data_pick_up As String, ByVal ore_pick_up As Integer, ByVal minuti_pick_up As Integer, ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal id_condizione_figlia As String, ByVal id_condizione_madre As String, ByVal id_gruppo As String, ByVal id_ditta As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String)
        'ANALISI DELLE RIGHE CALCOLABILI (tipo stampa 1 o 2) 
        Dim costo As String
        Dim percentuale As String
        Dim selezionato As String
    ...
    Public Function getListaStazioni() As String
        Try
            Dim Dbc As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            Dbc.Open()
            Dim sqlStr As String = "SELECT nome_stazione + ' - ' + indirizzo AS stazione, id, attiva, tipo_stazione FROM stazioni WHERE (id > 1) AND (attiva = 'True')  order by CodDollar "
            'Return sqlStr
            ...
            Do While Rs.Read()
                ...
                dictionary.Add("tipo_stazione", Rs("tipo_stazione"))    'aggiunta 17.03.2022 AP=APT / PT=Porto / DT=Città
    ...
    <WebMethod()>
    Public Function getElencoVeicoli(ByVal stazione_inizio_noleggio As String, ByVal data_inizio_noleggio As String, ByVal ora_inizio_noleggio As String,
                                     ByVal stazione_fine_noleggio As String, ByVal data_fine_noleggio As String, ByVal ora_fine_noleggio As String, ByVal eta As String,
                                     ByVal id_tip_cliente As Integer, ByVal lingua As String, ByVal cod_promo As String) As String
        ...
        Try
            ...
            Dim dtendsql As String = Year(data_fine_completa) & "-" & Month(data_fine_completa) & "-" & Day(data_fine_completa) & " 23:59:59"
            ...
            'vecchia posizione del codice #A spostato sotto per inserire controllo VAL 23.02.2022
            ''se flag di VAL abilitati restituisce Elenco vuoto 23.02.2022
            Dim response_VAL As Boolean = False
            Dim response_VAL_OFF As Boolean = False
            ...
            Else
                ...
                'verifica VAL 24.02.2022

                Dim FlagVal As Boolean = True   'default Abilitato

                'verifica condizioni sulle stazioni 24.02.2022
                'se stazione diversa verifica VAL
                If stazione <> stazione_off Then

                    Dim valStazioneInizio As String = funzioni_comuni.getStazioneVal(stazione)   'False#False o True#True
                    Dim valStazioneFine As String = funzioni_comuni.getStazioneVal(stazione_off)
                    Dim aValIni() As String = Split(valStazioneInizio, "#")
                    Dim aValEnd() As String = Split(valStazioneFine, "#")

                    'verifica se VAL stazione uscita verso altre stazioni
                    If aValIni(0) = "False" Then    'Non abilitato VAL da Stazione uscita vs Altre
                        FlagVal = False
                    End If
                    If aValIni(1) = "False" Then    'Non abilitato VAL da Altre Stazioni uscita vs Stazione Uscita
                        FlagVal = False
                    End If

                    If aValEnd(0) = "False" Then    'Non abilitato VAL da Altre Stazioni uscita vs Stazione Uscita
                        FlagVal = False
                    End If

                    If aValEnd(1) = "False" Then    'Non abilitato VAL da Stazioni uscita vs Stazione rientro
                        FlagVal = False
                    End If


                    If FlagVal = False Then 'non è possibile generare lista

                        'restituisce lista 
                        Dim listVal As New List(Of Dictionary(Of String, String))()
                        Dim dictionaryVal As New Dictionary(Of String, String)(1)

                        dictionaryVal = New Dictionary(Of String, String)(1)
                        dictionaryVal.Add("Stazione VAL OFF", "Lista Vuota")
                        listVal.Add(dictionaryVal)

                        Dim jVal As JavaScriptSerializer = New JavaScriptSerializer()
                        Return (jVal.Serialize(listVal.ToArray()))

                        Exit Function
                    End If


                End If 'se stazione diversa verifica VAL 24.02.2022
                ...
                sqla += "LEFT JOIN clienti_tipologia ON stop_sell_fonti.id_fonte = clienti_tipologia.id WHERE (convert(datetime,'" & dtinisql & "',102) "
                sqla += "BETWEEN da_data And a_data) Or (convert(datetime,'" & dtinisql & "',102) < da_data and convert(datetime,'" & dtendsql & "',102) >= da_data)"
                ...
                Do While Rs.Read()
                    ...
                    Else
                        ...
                        sqlstr += "And ((convert(datetime,'" & dtinisql & "',102)  BETWEEN da_data AND a_data) "
                        sqlstr += "Or (convert(datetime,'" & dtinisql & "',102) < da_data and convert(datetime,'" & dtendsql & "',102) >= da_data)) "
                ...
                Else
                    ...
                    If StazioneInStopSale = False Then
                        ...
                        If id_stop_sell <> "" Then
                            ...
                            sqlstr += "WHERE (convert(datetime,'" & dtinisql & "',102)  BETWEEN da_data AND a_data) "
                            sqlstr += "Or (convert(datetime,'" & dtinisql & "',102) < da_data and convert(datetime,'" & dtendsql & "',102) >= da_data)"
                            ...
                            Else
                                ...
                                Do While Rs2.Read()
                                    ...
                                    If (id_tariffe_righe & "") <> "" Or (id_tariffe_righe_prep & "") <> "" Then
                                        ...
                                        '# Aggiunta x test salvo 01.02.2023
                                        Dim tipoTariffa As String = "P" 'dal Web sono sempre Particolari
                                        Dim descTariffa As String = funzioni_comuni_new.GetCodiceTariffa(id_tariffe_righe) '"Web banco PMO Aeroporto" '2698  qui deve recuperare la descrizione della tariffa
                                        Dim descTariffaPren As String = funzioni_comuni_new.GetCodiceTariffa(id_tariffe_righe_prep) '
                                        Dim Data_Drop_off As String = data_fine_noleggio '"02/03/2023"
                                        Dim TipoCli As String = id_tip_cliente '"0"
                                        Dim Data_Creazione As String = Date.Now.ToShortDateString.ToString & " " & Date.Now.ToLongTimeString ' "01/02/2023 09:00:00"
                                        Dim max_sconto_new As String = "0"
                                        '@end salvo
                                        ...
                                        Dim esito As String = ""
                                        Dim esitoPre As String = ""

                                        esito = calcolaTariffa_x_gruppo(stazione2, data_inizio_completa, ora_inizio2, minuti_inizio2, stazione_off2, id_tariffe_righe, Rs2("ID_gruppo"), id_gruppo_rack, numero_giorni, giorni_prepagati_x_modifica,
                                                                prepagata, giorni_noleggio_extra_rack, sconto, tipo_Sconto, sconto_web_prepagato_primo_calcolo, sconto_su_rack, etaConducente, etaConducente, Id_nuovo_preventivo, id_ribaltamento,
                                                                id_prenotazione, id_contratto, num_calcolo, id_utente, id_ditta, id_fonte_commissionabile, commissione_percentuale, tipo_commissione, primo_calcolo_commissione,
                                                                giorni_commissioni_originale, "", tipoTariffa, descTariffa, Data_Drop_off, TipoCli, Data_Creazione, max_sconto_new)



                                        Thread.Sleep(100)
                                        esitoPre = calcolaTariffa_x_gruppo(stazione2, data_inizio_completa, ora_inizio2, minuti_inizio2, stazione_off2, id_tariffe_righe_prep, Rs2("ID_gruppo"), id_gruppo_rack, numero_giorni, giorni_prepagati_x_modifica,
                                                                prepagata_si, giorni_noleggio_extra_rack, sconto, tipo_Sconto, sconto_web_prepagato_primo_calcolo, sconto_su_rack, etaConducente, etaConducente, Id_nuovo_preventivo_prepagato,
                                                                id_ribaltamento, id_prenotazione, id_contratto, num_calcolo, id_utente, id_ditta, id_fonte_commissionabile, commissione_percentuale, tipo_commissione, primo_calcolo_commissione,
                                                                giorni_commissioni_originale, "", tipoTariffa, descTariffaPren, Data_Drop_off, TipoCli, Data_Creazione, max_sconto_new)
                                        ...
                                        '# salvo aggiungo per test salvo 01.02.2023
                                        'dictionary.Add("@dataORA", Date.Now.ToString)
                                        'dictionary.Add("@descTariffa", descTariffa)
                                        'dictionary.Add("@Data_Drop_off", Data_Drop_off)
                                        'dictionary.Add("@TipoCli", TipoCli)
                                        'dictionary.Add("@Data_Creazione", Data_Creazione)
                                        'dictionary.Add("@max_sconto_new", max_sconto_new)
                                        'dictionary.Add("@esito", esito)
                                        'dictionary.Add("@esitoPre", esitoPre)
                                        '@end 
                                        ...
                                        'deposito cauzionale del gruppo 31.03.2022 inserito in produzione il 12.04.2022
                                        Dim idGruppo As String = Rs2("ID_gruppo")

                                        Dim DepositoCauzionale As String
                                        'DepositoCauzionale = "800"
                                        'dictionary.Add("Deposito Cauzionale", DepositoCauzionale)

                                        'Modifica 04/05/2022 Tony
                                        'funzioni_comuni.GetValoreDepositoCauzionaleDefault(idGruppo)
                                        dictionary.Add("Deposito Cauzionale", GetValoreDepositoCauzionaleDefault(idGruppo))
        ...
        Try
            ...
            Do While Rs2.Read()
                ...
                If (id_tariffe_righe & "") <> "" Or (id_tariffe_righe_prep & "") <> "" Then
                    ...
                    'Inzio 05/06/2015      
                    dictionary.Add("Id Preventivo", Id_nuovo_preventivo)
                    dictionary.Add("Id Preventivo Prepagato", "12345678")
                    dictionary.Add("Stazione Uscita", "3")
                    dictionary.Add("Data PikUp", "28/06/2021")
                    dictionary.Add("Ora PikUp", "12")
                    dictionary.Add("Minuti PikUp", "30")
                    dictionary.Add("Stazione Entrata", "3")
                    dictionary.Add("Data DropOff", "30/06/2021")
                    dictionary.Add("Ora DropOff", "12")
                    dictionary.Add("Minuti DropOff", "30")
                    dictionary.Add("Tariffa Riga", id_tariffe_righe)
                    dictionary.Add("Tariffa RigaPrepagata", id_tariffe_righe_prep)
                    dictionary.Add("Id Gruppo", Rs2("ID_gruppo"))
                    dictionary.Add("Id Gruppo Rack", id_gruppo_rack)
                    dictionary.Add("Numero Giorni", "1")
                    dictionary.Add("Età", "55")
                    dictionary.Add("Giorni Prepagata modifica", giorni_prepagati_x_modifica)
                    dictionary.Add("Paga al Banco", getPrezzo(Id_nuovo_preventivo, Rs2("ID_gruppo")))
                    dictionary.Add("Paga On-line", getPrezzo(Id_nuovo_preventivo_prepagato, Rs2("ID_gruppo")))
                    'Fine 05/06/2015
    ...
    Public Function putDatiPrenotazione(ByVal Id As String, ByVal data_inizio_completa As String, ByVal data_fine_completa As String, ByVal prepagata As Boolean, ByVal ora_inizio As String, ByVal minuti_inizio As String, ByVal ora_fine As String, ByVal minuti_fine As String, ByVal id_tariffe_righe As String, ByVal id_gruppo As String, ByVal stazione As String, ByVal stazione_off As String, ByVal nome As String, ByVal cognome As String, ByVal data_nascita As String, ByVal email As String, ByVal indirizzo As String, ByVal telefono As String, ByVal num_volo_arrivo As String, ByVal num_volo_partenza As String, ByVal eta As String, ByVal numero_giorni As String) As String
        ...
        Try
            ...
            sqlStr2 += "'" & Id & "','" & id_operatore & "',convert(datetime,GetDate(),102),'" & prora_out & "','" & prora_pr & "','" & numero_giorni & "','" & cod & "','" & cod & "',"
    ...
    Public Function putDatiPrenotazionePrepagata(ByVal Id As String, ByVal data_inizio_completa As String, ByVal data_fine_completa As String, ByVal prepagata As Boolean, ByVal ora_inizio As String, ByVal minuti_inizio As String, ByVal ora_fine As String, ByVal minuti_fine As String, ByVal id_tariffe_righe As String, ByVal id_gruppo As String, ByVal stazione As String, ByVal stazione_off As String, ByVal nome As String, ByVal cognome As String, ByVal data_nascita As String, ByVal email As String, ByVal indirizzo As String, ByVal telefono As String, ByVal num_volo_arrivo As String, ByVal num_volo_partenza As String, ByVal eta As String, ByVal numero_giorni As String, ByVal importo_prepagato As String) As String
      ...
      " NULL,convert(datetime,GetDate(),102)," &
      ...
      "'" & volo_pr & "',NULL,NULL,'',NULL,NULL,NULL,NULL,NULL,'True','" & importo_prepagato & "','" & numero_giorni & "',NULL" & ",'" & id_gruppo & "')"
            ...
            Dim dnow As String = Year(Date.Now) & "-" & Month(Date.Now) & "-" & Day(Date.Now) & " " & Hour(Date.Now) & ":" & Minute(Date.Now) & ":00.000" '  funzioni_comuni.GetDataSql()
            ...
            " NR_BATCH,PER_IMPORTO, CONTABILIZZATO,TRANSATION_TYPE, CARD_TYPE, operazione_stornata, acquire_id, action_code, DATA_OPERAZIONE,TentativiPOS)" &
             " VALUES " &
            "('" & Nr_Contratto & "',convert(datetime,'" & dnow & "',102),'1011098650','4','0.00','" & stazione & "','','','','0000023','1',convert(datetime,GetDate(),102),'web','" & num_prenotazione & "','000018'," & importo_prepagato & ",'0','MAG','2','0','00000000002','000',convert(datetime,'" & funzioni_comuni.getDataDb_con_orario(Now) & "',102),'1')"
    ...
    Public Function CreaPDFTest(ByVal num_contratto As String) As String
        ...
        Dim allega As String = funzioni_comuni.AllegaContrattoDopoFirmaTest(num_contratto)
