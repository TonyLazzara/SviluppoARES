...
<WebService(Namespace:="http://ares.sicilyrentcar.it/")> _
<WebServiceBinding(ConformsTo:=WsiProfiles.BasicProfile1_1)> _
<Global.Microsoft.VisualBasic.CompilerServices.DesignerGenerated()> _
Public Class WebServiceBookingNew
    ...
    Function get_tariffa_x_righe(ByVal data_uscita As String, ByVal data_rientro As String, ByVal giorni_nolo As String, ByVal stazione As String, ByVal stazioneOff As String, ByVal IdTipCliente As Integer, ByVal cod_promo As String) As String
        ...
        Else
            ...
            Dim Sql = "(SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) " & _
           "INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa " & _
           "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '0') AND tariffe.attiva='1' " & condizione_id_prev & " AND '" & pickup_da_temp & "' BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " & _
           "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a " & _
           "AND tariffe.id NOT IN (SELECT DISTINCT id_tariffa FROM tariffe_X_stazioni WITH(NOLOCK) WHERE id_tariffa=tariffe.id) " & _
           "AND tariffe.id NOT IN (SELECT DISTINCT id_tariffa FROM tariffe_X_fonti WITH(NOLOCK) WHERE id_tariffa=tariffe.id) " & _
           "AND '" & pickup_a_temp & "' <= ISNULL(max_data_rientro, '3000-12-12 23:59:59')) ORDER BY codice"
            ...
            End If
            Dim SqlTariffeParticolari = "SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) " & _
            "INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa " & _
            "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '0') AND tariffe.attiva='1' " & condizione_id_prev & " AND '" & pickup_da_temp & "' BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " & _
            "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a " & _
            "AND '" & pickup_a_temp & "' <= ISNULL(max_data_rientro, '3000-12-12 23:59:59') " & _
            "AND ( (" & _
            "((tariffe.id IN (SELECT DISTINCT id_tariffa FROM tariffe_x_stazioni WITH(NOLOCK) WHERE id_stazione='" & stazione & "' AND tipo='PICK' AND id_tariffa=tariffe.id)) AND " & _
            "(tariffe.id IN (SELECT DISTINCT id_tariffa FROM tariffe_x_stazioni WITH(NOLOCK) WHERE id_stazione='" & stazioneOff & "' AND tipo='DROP' AND id_tariffa=tariffe.id))) " & _
            " OR " & _
            "((tariffe.id IN (SELECT DISTINCT id_tariffa FROM tariffe_x_stazioni WITH(NOLOCK) WHERE id_stazione='" & stazione & "' AND tipo='PICK' AND id_tariffa=tariffe.id)) AND " & _
            ...
            " OR " & _
            "((tariffe.id IN (SELECT DISTINCT id_tariffa FROM tariffe_x_stazioni WITH(NOLOCK) WHERE id_stazione='" & stazioneOff & "' AND tipo='DROP' AND id_tariffa=tariffe.id)) AND " & _
            ...
            If IdTipCliente > 0 Then
                ...
                SqlTariffeParticolari = SqlTariffeParticolari & "" & _
                "AND (tariffe.id IN (SELECT DISTINCT id_tariffa FROM tariffe_x_fonti WITH(NOLOCK) WHERE id_tipologia_cliente='" & IdTipCliente & "' AND id_tariffa=tariffe.id AND (valido_da IS NULL OR getDate()>=valido_da))" & _
    ...
    Function get_tariffa_x_righe_prepagato(ByVal data_uscita As String, ByVal data_rientro As String, ByVal giorni_nolo As String, ByVal stazione As String, ByVal stazioneOff As String, ByVal IdTipCliente As Integer, ByVal cod_promo As String) As String
        ...
        Else
            ...
            Dim Sql = "(SELECT top(1) tariffe_righe.id FROM tariffe WITH(NOLOCK) " & _
           "INNER JOIN tariffe_righe WITH(NOLOCK) ON tariffe.id=tariffe_righe.id_tariffa " & _
           "WHERE (tariffe.is_web = '1' AND tariffe.is_web_prepagato = '1') AND tariffe.attiva='1' " & condizione_id_prev & " AND '" & pickup_da_temp & "' BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " & _
           "AND GetDate() BETWEEN tariffe_righe.vendibilita_da AND tariffe_righe.vendibilita_a " & _
           "AND tariffe.id NOT IN (SELECT DISTINCT id_tariffa FROM tariffe_X_stazioni WITH(NOLOCK) WHERE id_tariffa=tariffe.id) " & _
           "AND tariffe.id NOT IN (SELECT DISTINCT id_tariffa FROM tariffe_X_fonti WITH(NOLOCK) WHERE id_tariffa=tariffe.id) " & _
           "AND '" & pickup_a_temp & "' <= ISNULL(max_data_rientro, '3000-12-12 23:59:59')) ORDER BY codice"
    ...
    Function nuovo_preventivo() As String
        ...
        Dim Dbc As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
        Dbc.Open()
        Dim Cmd As New Data.SqlClient.SqlCommand("INSERT INTO preventivi_web (id_operatore_creazione,data_creazione) VALUES('" & Costanti.costanti_web.id_operatore_web & "',getDate())", Dbc)
        Cmd.ExecuteNonQuery()
    ...
    Function nuovo_preventivo_prepagato() As String
        ...
        Dim Dbc As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
        Dbc.Open()
        Dim Cmd As New Data.SqlClient.SqlCommand("INSERT INTO preventivi_web (id_operatore_creazione,data_creazione) VALUES('5',getDate())", Dbc)
    ...
    Function getIdTempoKmRack(ByVal id_tariffe_righe As String, ByVal id_contratto As String, ByVal id_prenotazione As String) As String
        ...
        Else
            ...
            If id_tariffa_rack <> "" Then
                SqlStr = "SELECT tariffe_righe.id_tempo_km FROM tariffe_righe WITH(NOLOCK) " & _
                    "WHERE tariffe_righe.id_tariffa='" & id_tariffa_rack & "'  AND GetDate() BETWEEN tariffe_righe.pickup_da AND tariffe_righe.pickup_a " & _
            ...
            If id_elemento_val_figlia <> "" Or id_elemento_val_madre <> "" Then
                ...
                For i = 1 To 2
                    If (i = 1) Or (i = 2 And Not valore_trovato And id_condizione_madre <> "0") Then 'SE TROVO IL VALORE VAL PER LA CONDIZIONE FIGLIA NON LA CERCO PER LA MADRE
                        If i = 1 Then
                            sqlStr = "SELECT condizioni.iva_inclusa, condizioni_righe.id_metodo_stampa,condizioni_righe.obbligatorio,condizioni_elementi.descrizione As nome_val, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile, condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                            "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
                            "condizioni_righe.costo, condizioni_righe.tipo_costo FROM condizioni_righe WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) ON condizioni_elementi.id=condizioni_righe.id_elemento_val INNER JOIN aliquote_iva WITH(NOLOCK) ON condizioni_elementi.id_aliquota_iva=aliquote_iva.id LEFT OUTER JOIN condizioni_x_gruppi WITH(NOLOCK) ON " & _
                            "condizioni_x_gruppi.id_condizione=condizioni_righe.id INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') " & _
                            "AND (condizioni_righe.id_elemento_val='" & id_elemento_val & "') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                            "condizioni_x_gruppi.id_gruppo IS NULL) ORDER BY condizioni_x_gruppi.id_gruppo DESC"
                        ElseIf i = 2 Then
                            sqlStr = "SELECT condizioni.iva_inclusa, condizioni_righe.id_metodo_stampa, condizioni_righe.obbligatorio, condizioni_elementi.descrizione As nome_val, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile,condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                            "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
                            "condizioni_righe.costo, condizioni_righe.tipo_costo FROM condizioni_righe WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) ON condizioni_elementi.id=condizioni_righe.id_elemento_val INNER JOIN aliquote_iva WITH(NOLOCK) ON condizioni_elementi.id_aliquota_iva=aliquote_iva.id LEFT OUTER JOIN condizioni_x_gruppi WITH(NOLOCK) ON " & _
                            "condizioni_x_gruppi.id_condizione=condizioni_righe.id INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id WHERE (condizioni_righe.id_condizione='" & id_condizione_madre & "') " & _
                            "AND (condizioni_righe.id_elemento_val='" & id_elemento_val & "') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                            "condizioni_x_gruppi.id_gruppo IS NULL) ORDER BY condizioni_x_gruppi.id_gruppo DESC"
                        End If
            ...
            For i = 1 To 2
                If (i = 1) Or (i = 2 And Not valore_trovato And id_condizione_madre <> "0") Then 'SE TROVO IL COSTO DELL'ELEMENTO JOUNG DRIVER PER LA CONDIZIONE FIGLIA NON LA CERCO PER LA MADRE
                    If i = 1 Then
                        ...
                        "condizioni_x_gruppi.id_condizione=condizioni_righe.id INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') " & _
                        "AND (condizioni_elementi.tipologia='JOUNG') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                        "condizioni_x_gruppi.id_gruppo IS NULL) ORDER BY condizioni_x_gruppi.id_gruppo DESC"
                    ElseIf i = 2 Then
                        sqlStr = "SELECT condizioni.iva_inclusa,condizioni_righe.id_elemento, condizioni_righe.id_metodo_stampa, condizioni_righe.obbligatorio, condizioni_elementi.descrizione As nome_elemento, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile,condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                        "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
    ...
    Sub calcolaOnere(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean, ByVal elementi_prepagati As Collection, ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal id_condizione_figlia As String, ByVal id_condizione_madre As String, ByVal id_gruppo As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String)
        ...
        For i = 1 To 2
            If (i = 1) Or (i = 2 And Not valore_trovato And id_condizione_madre <> "0") Then 'SE TROVO IL VALORE ONERE PER LA CONDIZIONE FIGLIA NON LA CERCO PER LA MADRE
                If i = 1 Then
                    sqlStr = "SELECT condizioni.iva_inclusa, condizioni_righe.id_metodo_stampa,condizioni_righe.obbligatorio,condizioni_elementi.descrizione As nome_onere, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile, condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                    "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
                    "condizioni_righe.costo, condizioni_righe.tipo_costo FROM condizioni_righe WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) ON condizioni_elementi.id=condizioni_righe.id_elemento INNER JOIN aliquote_iva WITH(NOLOCK) ON condizioni_elementi.id_aliquota_iva=aliquote_iva.id LEFT OUTER JOIN condizioni_x_gruppi WITH(NOLOCK) ON " & _
                    "condizioni_x_gruppi.id_condizione=condizioni_righe.id INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') " & _
                    "AND (condizioni_righe.id_elemento='" & id_onere & "') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                    "condizioni_x_gruppi.id_gruppo IS NULL) ORDER BY condizioni_x_gruppi.id_gruppo DESC"
                ElseIf i = 2 Then
                    sqlStr = "SELECT condizioni.iva_inclusa, condizioni_righe.id_metodo_stampa, condizioni_righe.obbligatorio, condizioni_elementi.descrizione As nome_onere, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva,  condizioni_elementi.scontabile,condizioni_elementi.omaggiabile,condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                    "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
    ...
    Sub analisi_condizioni(ByVal giorni_noleggio As Integer, ByVal giorni_prepagati As Integer, ByVal prepagata As Boolean, ByVal elementi_prepagati As Collection, ByVal data_pick_up As String, ByVal ore_pick_up As Integer, ByVal minuti_pick_up As Integer, ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal id_condizione_figlia As String, ByVal id_condizione_madre As String, ByVal id_gruppo As String, ByVal id_ditta As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String)
        ...
        sqlStr = "(SELECT condizioni.iva_inclusa, condizioni_elementi.id As id_elemento, condizioni_elementi.tipologia_franchigia, condizioni_righe.id_metodo_stampa,condizioni_righe.obbligatorio,condizioni_elementi.descrizione As nome_elemento, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile, condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso ,condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                            "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
                            "condizioni_righe.costo, condizioni_righe.tipo_costo, condizioni_elementi.tipologia, condizioni_elementi.valorizza, condizioni_unita_misura.km_divisore_num_giorni, condizioni_unita_misura.descrizione As unita_misura " & _
                            "FROM condizioni_righe WITH(NOLOCK) " & _
                            "INNER JOIN condizioni_elementi WITH(NOLOCK) ON condizioni_righe.id_elemento=condizioni_elementi.id " & _
                            "INNER JOIN aliquote_iva WITH(NOLOCK) ON condizioni_elementi.id_aliquota_iva=aliquote_iva.id " & _
                            "LEFT OUTER JOIN condizioni_x_gruppi WITH(NOLOCK) ON condizioni_x_gruppi.id_condizione=condizioni_righe.id " & _
                            "LEFT OUTER JOIN condizioni_unita_misura WITH(NOLOCK) ON condizioni_righe.id_unita_misura=condizioni_unita_misura.id " & _
                            "INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id " & _
                            "WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                            "condizioni_x_gruppi.id_gruppo IS NULL) AND (condizioni_elementi.tipologia='CONDIZIONE' OR condizioni_elementi.tipologia='KM_EXTRA' " & condizione_fuori_orario_pick_up & condizione_franchigie_ridotte & condizione_spese_postali & ") AND NOT (condizioni_righe.obbligatorio='0' AND condizioni_elementi.valorizza='0')) " & _
                            " UNION " & _
                "(SELECT condizioni.iva_inclusa, condizioni_elementi.id As id_elemento, condizioni_elementi.tipologia_franchigia, condizioni_righe.id_metodo_stampa,condizioni_righe.obbligatorio,condizioni_elementi.descrizione As nome_elemento, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva, condizioni_elementi.scontabile, condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                            ...
                            "LEFT OUTER JOIN condizioni_unita_misura WITH(NOLOCK) ON condizioni_righe.id_unita_misura=condizioni_unita_misura.id " & _
                            "INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id " & _
                            " WHERE (condizioni_elementi.tipologia='CONDIZIONE' OR condizioni_elementi.tipologia='KM_EXTRA' " & condizione_fuori_orario_pick_up & condizione_franchigie_ridotte & condizione_spese_postali & ")" & _
                            " AND (condizioni_righe.id_condizione='" & id_condizione_madre & "') " & _
                            " AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR condizioni_x_gruppi.id_gruppo IS NULL) " & _
                            " AND (NOT (condizioni_righe.obbligatorio='0' AND condizioni_elementi.valorizza='0')) " & _
                            " AND (condizioni_elementi.id NOT IN (SELECT DISTINCT ISNULL(condizioni_righe.id_elemento,0) FROM condizioni_righe WITH(NOLOCK) LEFT OUTER JOIN condizioni_x_gruppi WITH(NOLOCK) ON condizioni_x_gruppi.id_condizione=condizioni_righe.id WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR condizioni_x_gruppi.id_gruppo IS NULL)))" & _
    ...
    Sub aggiungi_accessorio_pieno_caburante(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String, ByVal id_gruppo As String)
        ...
        Dim SqlStr As String = "SELECT condizioni_elementi.id, condizioni_elementi.descrizione, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva," & _
        "scontabile, omaggiabile, acquistabile_nolo_in_corso FROM condizioni_elementi WITH(NOLOCK)" & _
        "INNER JOIN aliquote_iva WITH(NOLOCK) ON condizioni_elementi.id_aliquota_iva=aliquote_iva.id " & _
    ...
    Sub calcola_commissioni_gia_pagate(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal id_gruppo As Integer, ByVal num_calcolo As Integer, ByVal commissione_percentuale As Double, ByVal primo_calcolo As Boolean)
        ...
        If primo_calcolo Then
            campi_primo_calcolo = ", commissioni_imponibile_originale=((ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*100/(100+aliquota_iva))," & _
            "commissioni_iva_originale=((ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*aliquota_iva/(100+aliquota_iva)) "
        End If

        Dim sqlStr As String = "UPDATE " & tabella & " SET commissioni_imponibile=((ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*100/(100+aliquota_iva))," & _
            "commissioni_iva=((ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*aliquota_iva/(100+aliquota_iva)) " & campi_primo_calcolo & " " & _
        ...
        campi_primo_calcolo = ""
        If primo_calcolo Then
            campi_primo_calcolo = ", commissioni_imponibile_originale=(ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)," & _
            "commissioni_iva_originale=(ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*aliquota_iva/100 "
        End If

        sqlStr = "UPDATE " & tabella & " SET commissioni_imponibile=(ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)," & _
            "commissioni_iva=(ISNULL(valore_costo,0)*" & Replace(commissione_percentuale, ",", ".") & "/100)*aliquota_iva/100" & campi_primo_calcolo & " " & _
        ...
        If Not primo_calcolo Then
            ...
            sqlStr = "UPDATE t1 SET commissioni_imponibile_originale=t2.commissioni_imponibile_originale, commissioni_iva_originale=t2.commissioni_iva_originale " & _
            "FROM " & tabella & " AS t1 " & _
            "INNER JOIN " & tabella & " AS t2 ON t1.id_documento=t2.id_documento AND t1.id_gruppo=t2.id_gruppo AND t1.id_elemento=t2.id_elemento  " & _
    ...
    Sub riporta_commissioni_agenzia(ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As Integer, ByVal id_gruppo As Integer)
        ...
        Dim sqlStr As String = "UPDATE t1 SET commissioni_imponibile=t2.commissioni_imponibile_originale, " & _
            "commissioni_iva=t2.commissioni_iva_originale, commissioni_imponibile_originale=t2.commissioni_imponibile_originale, commissioni_iva_originale=t2.commissioni_iva_originale " & _
            "FROM " & tabella & " AS t1 " & _
            "INNER JOIN " & tabella & " AS t2 ON t1.id_documento=t2.id_documento AND t1.id_gruppo=t2.id_gruppo AND t1.id_elemento=t2.id_elemento  " & _
            "WHERE t1.id_documento=" & id_da_cercare & " AND t1.num_calcolo=" & num_calcolo & " AND t2.num_calcolo=" & num_calcolo - 1 & " AND t1.id_gruppo=" & id_gruppo & " AND t1.id_elemento=" & Costanti.ID_tempo_km
    ...
    Function gps_obbligatorio_o_incluso(ByVal id_gruppo As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String) As Boolean
        ...
        Dim sqlStr As String = "SELECT " & tabella & ".id FROM " & tabella & " WITH(NOLOCK) " & _
            "INNER JOIN condizioni_elementi WITH(NOLOCK) ON " & tabella & ".id_elemento=condizioni_elementi.id " & _
            "WHERE condizioni_elementi.is_gps='1' " & _
            "AND ((" & tabella & ".obbligatorio='1' AND " & tabella & ".id_a_carico_di='" & Costanti.id_a_carico_del_cliente & "' AND " & tabella & ".id_metodo_stampa='" & Costanti.id_valorizza_nel_contratto & "') " & _
            "OR (" & tabella & ".id_a_carico_di='" & Costanti.id_accessorio_incluso & "')) AND " & tabella & ".id_documento='" & id_da_cercare & "' " & _
    ...
    Sub aggiungi_accessori_obbligatori_prepagati_calcolo_precedente(ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As Integer, ByVal id_gruppo As String, ByVal stazione_pick_up As String, ByVal stazione_drop_off As String, ByVal giorni As Integer, ByVal giorni_prepagati As Integer, ByVal sconto As Double, ByVal id_tariffe_righe As Integer, ByVal id_utente As String)
        ...
        Dim sqlStr As String = "SELECT " & tabella & ".id_elemento, " & tabella & ".num_elemento, condizioni_elementi.tipologia, valore_costo FROM " & tabella & " WITH(NOLOCK) " & _
            " INNER JOIN condizioni_elementi WITH(NOLOCK) ON " & tabella & ".id_elemento=condizioni_elementi.id " & _
            " WHERE " & tabella & ".id_documento=" & id_da_cercare & _
            " AND " & tabella & ".num_calcolo=" & num_calcolo - 1 & " AND id_a_carico_di <> " & Costanti.id_accessorio_incluso & _
            " AND obbligatorio='1' AND id_metodo_stampa<>" & Costanti.id_stampa_informativa_con_valore & " AND id_metodo_stampa<>" & Costanti.id_stampa_informativa_senza_valore & _
            " AND NOT id_elemento IS NULL AND prepagato='1' " & _
            " AND id_elemento + '-' + ISNULL(num_elemento,'') NOT IN (SELECT " & tabella & ".id_elemento + '-' + ISNULL(num_elemento,'') FROM " & tabella & " WHERE " & tabella & ".id_documento=" & id_da_cercare & _
            " AND " & tabella & ".num_calcolo=" & num_calcolo & " AND id_a_carico_di <> " & Costanti.id_accessorio_incluso & _
    ...
    Sub prepagato_memorizza_costi_prepagati_x_fattura(ByVal id_preventivo As String, ByVal id_prenotazione As String, ByVal id_ribaltamento As String, ByVal id_contratto As String, ByVal id_gruppo As Integer, ByVal num_calcolo As Integer, ByVal id_elemento As String)
        ...
        Dim sqlStr As String = "UPDATE " & tabella & " SET imponibile_scontato_prepagato=imponibile_scontato, iva_imponibile_scontato_prepagato=iva_imponibile_scontato," & _
            "imponibile_onere_prepagato=imponibile_onere, iva_onere_prepagato=iva_onere WHERE id_documento=" & id_da_cercare & " AND num_calcolo=" & num_calcolo & _
    ...
    Sub aggiorna_commissioni_da_riconoscere(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal id_gruppo As Integer, ByVal num_calcolo As Integer, ByVal id_fonte_commissionabile As Integer, ByVal commissione_percentuale As Double)
        ...
        Dim sqlStr As String = "UPDATE " & tabella & " SET commissioni_imponibile_originale=ISNULL(imponibile_scontato,0)*" & Replace(CDbl(commissione_percentuale), ",", ".") & "/100," & _
            "commissioni_iva_originale=ISNULL(iva_imponibile_scontato,0)*" & Replace(CDbl(commissione_percentuale), ",", ".") & "/100 WHERE id_documento=" & id_da_cercare & " AND num_calcolo=" & num_calcolo & _
        ...
        sqlStr = "UPDATE " & tabella & " SET commissioni_imponibile_originale=(SELECT SUM(ISNULL(commissioni_imponibile_originale,0)) FROM " & tabella & " WITH(NOLOCK) " & _
            "WHERE id_documento=" & id_da_cercare & " AND num_calcolo=" & num_calcolo & " AND id_gruppo=" & id_gruppo & " AND selezionato='1' AND nome_costo<>'" & Replace(Costanti.testo_elemento_totale, "'", "''") & "'), " & _
            "commissioni_iva_originale=(SELECT SUM(ISNULL(commissioni_iva_originale,0)) FROM " & tabella & " WITH(NOLOCK) " & _
            "WHERE id_documento=" & id_da_cercare & " AND num_calcolo=" & num_calcolo & " AND id_gruppo=" & id_gruppo & " AND selezionato='1' AND nome_costo<>'" & Replace(Costanti.testo_elemento_totale, "'", "''") & "') " & _
    ...
    Function stazione_aperta_pick_up(ByVal id_stazione As String, ByVal data As String, ByVal ore As String, ByVal minuti As String) As String
        ...
        Dim Cmd As New Data.SqlClient.SqlCommand("SELECT id_orario FROM stazione_orari WITH(NOLOCK) WHERE (CAST('" & data & "' AS DateTime) BETWEEN CAST(CAST(da_giorno AS nvarchar(4)) + '/' + CAST(da_mese AS nvarchar(4)) + '/' + '" & anno & "' AS DateTime) AND CAST(CAST(a_giorno AS nvarchar(4)) + '/' + CAST(a_mese AS nvarchar(4)) + '/' + '" & anno & "' AS DateTime)) AND id_stazione='" & id_stazione & "'", Dbc)
        ...
        Else
            ...
            Else
                ...
                Cmd = New Data.SqlClient.SqlCommand("SELECT opening_hour_from,opening_minute_from,opening_hour_to,opening_minute_to,ISNULL(opening_hour_from2,'-1') As opening_hour_from2,ISNULL(opening_minute_from2,'-1') As opening_minute_from2, ISNULL(opening_hour_to2,'-1') As opening_hour_to2, ISNULL(opening_minute_to2,'-1') As opening_minute_to2 FROM orario_settimanale_righe WITH(NOLOCK) WHERE id_orario_settimanale='" & id_orario_stazione & "' AND '" & giorno_settimana & "' BETWEEN opening_day_from AND opening_day_to", Dbc)
                Rs = Cmd.ExecuteReader()
                Rs.Read()
    ...
    Function get_elemento_fuori_orario_pickUp(ByVal ore_pick_up As Integer, ByVal minuti_pick_up As Integer) As String
        ...
        Dim sqlStr As String = "SELECT id FROM condizioni_elementi WITH(NOLOCK) WHERE tipologia='FUORI' AND" & _
        "((" & ore_pick_up & " > ore_inizio_fuori_orario AND " & ore_pick_up & " < ore_fine_fuori_orario) " & _
        " OR (" & ore_pick_up & "=ore_inizio_fuori_orario AND " & minuti_pick_up & ">= minuti_inizio_fuori_orario) " & _
            ...
            For i = 1 To 2
                If (i = 1) Or (i = 2 And Not valore_trovato And id_condizione_madre <> "0") Then 'SE TROVO IL COSTO DELL'ELEMENTO JOUNG DRIVER PER LA CONDIZIONE FIGLIA NON LA CERCO PER LA MADRE
                    ...
                    If i = 1 Then
                        ...
                        "condizioni_x_gruppi.id_condizione=condizioni_righe.id INNER JOIN condizioni WITH(NOLOCK) ON condizioni_righe.id_condizione=condizioni.id WHERE (condizioni_righe.id_condizione='" & id_condizione_figlia & "') " & _
                        "AND (condizioni_elementi.id='" & id_accessorio & "') AND condizioni_righe.obbligatorio='" & extra_od_obbligatorio & "' AND (condizioni_x_gruppi.id_gruppo='" & id_gruppo & "' OR " & _
                        "condizioni_x_gruppi.id_gruppo IS NULL) ORDER BY condizioni_x_gruppi.id_gruppo DESC"
                    ElseIf i = 2 Then
                        sqlStr = "SELECT condizioni.iva_inclusa,condizioni_righe." & colonna_elemento & ", condizioni_righe.id_metodo_stampa, condizioni_righe.obbligatorio, condizioni_elementi.descrizione As nome_elemento, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva,  condizioni_elementi.scontabile,condizioni_elementi.omaggiabile, condizioni_elementi.acquistabile_nolo_in_corso, condizioni_righe.id, condizioni_x_gruppi.id_gruppo, condizioni_righe.applicabilita_da," & _
                        "condizioni_righe.applicabilita_a, condizioni_righe.id_a_carico_di,condizioni_righe.pac, condizioni_righe.id_unita_misura," & _
    ...
    Sub calcolo_iva_e_totale_singolo_accessorio(ByVal stazione_pick_up As String, ByVal sconto As Double, ByVal id_gruppo As String, ByVal id_accessorio As String, ByVal num_elemento As String, ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String, ByVal imposta_prepagato As Boolean, ByVal tipo_commissione As String, ByVal percentuale_commissioni As String, ByVal id_fonte_commissionabile As String)
        ...
        If elemento_commissionabile Then
            ...
            update_commissioni = ", commissioni_imponibile_originale=ISNULL(commissioni_imponibile_originale,0)+" & Replace((totale_imponibile - imponibile_percentuale) * CDbl(percentuale_commissioni) / 100, ",", ".") & _
    ...
    Sub copia_arrotondamento_prepagato(ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal id_gruppo As String, ByVal num_calcolo As String, ByVal differenza_costo As Double)
        ...
        Dim Dbc As New Data.SqlClient.SqlConnection(Web.Configuration.WebConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
        Dbc.Open()
        Dim sqlStr As String = "SELECT TOP 1 condizioni_elementi.id, condizioni_elementi.descrizione, aliquote_iva.aliquota As iva, aliquote_iva.codice_iva FROM condizioni_elementi WITH(NOLOCK) " & _
            "INNER JOIN aliquote_iva WITH(NOLOCK) ON aliquote_iva.id=condizioni_elementi.id_aliquota_iva " & _
        ...
        sqlStr = "UPDATE " & tabella & " SET imponibile_scontato_prepagato=imponibile_scontato, iva_imponibile_scontato_prepagato=iva_imponibile_scontato," & _
            "imponibile_onere_prepagato=imponibile_onere, iva_onere_prepagato=iva_onere WHERE id_documento=" & id_da_cercare & " AND num_calcolo=" & num_calcolo & _
    ...
    Public Shared Sub aggiungi_costo_accessorio(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazioni As String, ByVal id_contratto As String, ByVal num_calcolo As String, ByVal id_gruppo As String, ByVal id_elemento As String, ByVal giorni_da_calcolare_x_nolo_in_corso As String, ByVal sconto_x_nolo_in_corso As String, ByVal data_aggiunta_x_nolo_in_corso As String, ByVal imposta_prepagato As Boolean, ByVal tipo_commissione As String, ByVal percentuale_commissione As String, ByVal id_fonte_commissionabile As String)
        ...
        If imposta_prepagato Then
            condizione_prepagato = ", prepagato=1, imponibile_scontato_prepagato=imponibile_scontato, iva_imponibile_scontato_prepagato=iva_imponibile_scontato, " & _
        ...
        If Rs.Read() Then
            ...
            If elemento_commissionabile Then
                'SE L'ELEMENTO E' COMMISSIONABILE AUMENTO, NEL TOTALE, I VALORI DELLE COMMISSIONI
                update_commissioni = ", commissioni_imponibile_originale=ISNULL(commissioni_imponibile_originale,0)+" & Replace(imponibile_elemento * CDbl(percentuale_commissione) / 100, ",", ".") & _
            ...
            If elemento_commissionabile Then
                update_commissioni = ", commissioni_imponibile_originale=ISNULL(imponibile_scontato,0)*" & Replace(CDbl(percentuale_commissione), ",", ".") & "/100," & _
    ...
    Private Shared Sub aggiorna_franchigie(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazione As String, ByVal id_contratto As String, ByVal num_calcolo As String, ByVal id_gruppo As String, ByVal sottotipologia_franchigia As String)
        ...
        If sottotipologia_franchigia <> "TOTALE" Then
            ...
            If tabella = "prenotazioni_costi" Or tabella = "contratti_costi" Then
                ...
                Dim sqlStr As String = "SELECT ISNULL(prepagato,0) FROM " & tabella & " WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) " & _
                    "ON " & tabella & ".id_elemento=condizioni_elementi.id WHERE tipologia_franchigia='ASSICURAZIONE' AND sottotipologia_franchigia='TOTALE' " & _
        ...
        Else
            ...
            If tabella = "prenotazioni_costi" Or tabella = "contratti_costi" Then
                ...
                'INVECE CHE SULLE FRANCHIGIE COMPLETE COME NEGLI ALTRI CASI
                Dim sqlStr As String = "SELECT ISNULL(prepagato,0) FROM " & tabella & " WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) " & _
                    "ON " & tabella & ".id_elemento=condizioni_elementi.id WHERE tipologia_franchigia='ASSICURAZIONE' AND sottotipologia_franchigia='DANNI' " & _
                    "AND id_documento='" & id_documento & "' AND num_calcolo='" & num_calcolo & "' AND id_gruppo='" & id_gruppo & "'"
                ...
                sqlStr = "SELECT ISNULL(prepagato,0) FROM " & tabella & " WITH(NOLOCK) INNER JOIN condizioni_elementi WITH(NOLOCK) " & _
                    "ON " & tabella & ".id_elemento=condizioni_elementi.id WHERE tipologia_franchigia='ASSICURAZIONE' AND sottotipologia_franchigia='FURTO' " & _
                    "AND id_documento='" & id_documento & "' AND num_calcolo='" & num_calcolo & "' AND id_gruppo='" & id_gruppo & "'"
    ...
    Shared Sub rimuovi_costo_accessorio(ByVal id_preventivo As String, ByVal id_ribaltamento As String, ByVal id_prenotazioni As String, ByVal id_contratto As String, ByVal num_calcolo As String, ByVal id_gruppo As String, ByVal id_elemento As String, ByVal giorni_da_calcolare_x_nolo_in_corso As String, ByVal sconto_x_nolo_in_corso As String, ByVal data_aggiunta_x_nolo_in_corso As String, ByVal imposta_prepagato As Boolean, ByVal tipo_commissione As String, ByVal percentuale_commissione As String, ByVal id_fonte_commissionabile As String)
        ...
        Dim condizione_prepagato As String = ""
        If imposta_prepagato Then
            condizione_prepagato = ", prepagato=0, imponibile_scontato_prepagato=imponibile_scontato, iva_imponibile_scontato_prepagato=iva_imponibile_scontato, " & _
                "imponibile_onere_prepagato=imponibile_onere, iva_onere_prepagato=iva_onere "
        End If
    ...
    Public Function getElencoVeicoli(ByVal stazione_inizio_noleggio As String, ByVal data_inizio_noleggio As String, ByVal ora_inizio_noleggio As String, ByVal stazione_fine_noleggio As String, ByVal data_fine_noleggio As String, ByVal ora_fine_noleggio As String, ByVal eta As String, ByVal id_tip_cliente As Integer, ByVal lingua As String, ByVal cod_promo As String) As String
        ...
        Try
            ...
            Dim SqlPulisciDB As String = "SELECT * FROM preventivi_web  WITH(NOLOCK)  WHERE (data_creazione  < convert(datetime, convert(varchar(20), day(getdate())) + '-' + convert(varchar(20), month(getdate())) + '-' + convert(varchar(20), year(getdate())), 105)) order by id desc"
            ...
            Dim SqlPulisciDB2 As String = "delete FROM preventivi_web  WHERE (data_creazione  < convert(datetime, convert(varchar(20), day(getdate())) + '-' + convert(varchar(20), month(getdate())) + '-' + convert(varchar(20), year(getdate())), 105))"
            ...
            Dim Dbc As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            Dbc.Open()
            Dim Cmd As New Data.SqlClient.SqlCommand("SELECT stop_sell.id as id_stop_sell,* FROM stop_sell LEFT JOIN stop_sell_fonti ON stop_sell.id = stop_sell_fonti.id_stop_sell LEFT JOIN clienti_tipologia ON stop_sell_fonti.id_fonte = clienti_tipologia.id   WHERE '" & data_inizio_completa & "'  BETWEEN da_data AND a_data", Dbc)
            QueryError = Cmd.CommandText
            ...
            QueryError = stazione_aperta_pickup & " " & stazione_aperta_dropOff
            ...
            Else
                ...
                Do While Rs.Read()
                    ...
                    Else
                        ...
                        Dbc2.Open()
                        Dim Cmd2 As New Data.SqlClient.SqlCommand("SELECT stop_sell.id as id_stop_sell,stop_sell.*,stop_sell_stazioni.id_stazione FROM stop_sell,stop_sell_stazioni where stop_sell.id = stop_sell_stazioni.id_stop_sell and '" & data_inizio_completa & "'  BETWEEN da_data AND a_data and id_stazione = " & stazione2, Dbc2)
                        QueryError = Cmd2.CommandText
            ...
            End If
        Catch ex As Exception
            Return ex.Message & " QueryError: " & QueryError
        End Try
    End Function
    ...
    Public Function getListaExtra(ByVal IdPrepagato As String, ByVal lingua As String) As String
        Try
            ...
            Return (j.Serialize(list.ToArray()))
        Catch ex As Exception
            Return "Errore Lista Extra " & ex.Message
        End Try
    End Function
    ...
    Public Function getListaCondizioniInclusi(ByVal Id As String, ByVal lingua As String) As String
        Try
            ...
            DbcCondizioni.Open()
            Dim CmdCondizioni As New Data.SqlClient.SqlCommand("SELECT condizioni_elementi.descrizione_en,nome_costo, ISNULL((imponibile+iva_imponibile+ISNULL(imponibile_onere,0)+ISNULL(iva_onere,0)),0) As valore_costo, ISNULL((imponibile+iva_imponibile-imponibile_scontato-iva_imponibile_scontato),0) As sconto FROM preventivi_web_costi,condizioni_elementi WITH(NOLOCK) WHERE preventivi_web_costi.nome_costo =  condizioni_elementi.descrizione and ((id_documento = '" & Id & "') AND id_elemento > 0 AND id_elemento <> '" & id_elemento_escluso & "' AND obbligatorio='True' AND id_metodo_stampa= '" & id_metodo_stampa & "') ORDER BY nome_costo ASC", DbcCondizioni)
    ...
    Public Function getListaFranchigie(ByVal Id As String, ByVal lingua As String) As String
        Try
            ...
            DbcFranchiggie.Open()
            Dim CmdFranchiggie As New Data.SqlClient.SqlCommand("SELECT condizioni_elementi.descrizione_en,nome_costo, ISNULL((imponibile+iva_imponibile+ISNULL(imponibile_onere,0)+ISNULL(iva_onere,0)),0) As valore_costo, ISNULL((imponibile+iva_imponibile-imponibile_scontato-iva_imponibile_scontato),0) As sconto FROM preventivi_web_costi,condizioni_elementi WITH(NOLOCK) WHERE preventivi_web_costi.nome_costo =  condizioni_elementi.descrizione and ((id_documento = '" & Id & "') AND id_elemento > 0 AND id_elemento <> '" & id_elemento_escluso & "' AND franchigia_attiva = 'True')  ORDER BY nome_costo ASC", DbcFranchiggie)
    ...
    Public Function getNomiFranchigie(ByVal Id As String, ByVal lingua As String) As String
        Try
            ...
            Dim DbcFranchiggie As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("SicilyConnectionString").ConnectionString)
            DbcFranchiggie.Open()
            Dim CmdFranchiggie As New Data.SqlClient.SqlCommand("SELECT condizioni_elementi.descrizione,condizioni_elementi.descrizione_en,condizioni_elementi.ordinamento_vis from condizioni_elementi  WITH(NOLOCK) where 1=1 and condizioni_elementi.tipologia_franchigia = 'FRANCHIGIA' ORDER BY id ASC", DbcFranchiggie)
            ...
            Return (j.Serialize(list.ToArray()))
        Catch ex As Exception
            Return "Errore in Nomi Franchigie"
        End Try
    End Function
    ...
    Public Function putDatiPrenotazione(ByVal Id As String, ByVal data_inizio_completa As String, ByVal data_fine_completa As String, ByVal prepagata As Boolean, ByVal ora_inizio As String, ByVal minuti_inizio As String, ByVal ora_fine As String, ByVal minuti_fine As String, ByVal id_tariffe_righe As String, ByVal id_gruppo As String, ByVal stazione As String, ByVal stazione_off As String, ByVal nome As String, ByVal cognome As String, ByVal data_nascita As String, ByVal email As String, ByVal indirizzo As String, ByVal telefono As String, ByVal num_volo_arrivo As String, ByVal num_volo_partenza As String, ByVal eta As String, ByVal numero_giorni As String) As String
        Try
            ...
            If data_nascita = "" Then
                data_nascita = "NULL"
            Else
                data_nascita = funzioni_comuni.getDataDb_senza_orario(data_nascita)
            End If
            ...
            Dim sqlStr = "INSERT INTO prenotazioni_costi (id_documento, num_calcolo, ordine_stampa, id_gruppo, id_elemento, num_elemento, nome_costo," & _
    ...
    Public Function putDatiPrenotazionePrepagata(ByVal Id As String, ByVal data_inizio_completa As String, ByVal data_fine_completa As String, ByVal prepagata As Boolean, ByVal ora_inizio As String, ByVal minuti_inizio As String, ByVal ora_fine As String, ByVal minuti_fine As String, ByVal id_tariffe_righe As String, ByVal id_gruppo As String, ByVal stazione As String, ByVal stazione_off As String, ByVal nome As String, ByVal cognome As String, ByVal data_nascita As String, ByVal email As String, ByVal indirizzo As String, ByVal telefono As String, ByVal num_volo_arrivo As String, ByVal num_volo_partenza As String, ByVal eta As String, ByVal numero_giorni As String, ByVal importo_prepagato As String) As String
      ...
      "cognome_conducente,eta_primo_guidatore,eta_secondo_guidatore,data_nascita,mail_conducente,indirizzo_conducente,riferimento_telefono,id_fonte,codice_edp,id_cliente,id_tariffa,id_tariffe_righe,tipo_tariffa,pren_broker_no_tariffa,sconto_applicato, tipo_sconto," & _
      "id_preventivo,id_operatore_creazione,DATAPREN,PRORA_OUT,PRORA_PR,giorni,CODTAR,codice,TAR_VAL_DAL,TAR_VAL_AL,N_VOLOOUT,N_VOLOPR," & _
      "IMPORTO_PREVENTIVO, importo_a_carico_del_broker, rif_to, giorni_to, id_fonte_commissionabile, tipo_commissione, commissione_percentuale, giorni_commissioni, prepagata, importo_prepagato, giorni_prepagati, sconto_web_prepagato)" & _
      " VALUES " & _
      "('" & num_prenotazione & "','1',0,'1','web','" & stazione & "','" & stazione & "','" & stazione_off & "'," & _
      "'" & data_uscita & "','" & ora_inizio & "','" & minuti_inizio & "','" & data_rientro & "'," & _
      "'" & ora_fine & "','" & minuti_fine & "','" & id_gruppo & "'," & _
      ...
      "'" & Replace(nome, "'", "''") & "','" & Replace(cognome, "'", "''") & "'," & _
      "'" & eta & "','','" & data_nascita & "','" & Replace(email, "'", "''") & "','" & Replace(indirizzo, "'", "''") & "','" & Replace(telefono, "'", "''") & "'," & _
      "'" & tipo_cliente & "',NULL," & id_ditta & "," & id_tariffa & "," & id_tariffe_righe & ",'" & tipo_tariffa & "'," & pren_no_tariffa & ",'0','0','" & id_operatore & "'," & _
      " NULL,GetDate()," & _
      "'" & prora_out & "','" & prora_pr & "','" & numero_giorni & "','" & cod & "','" & cod & "'," & _
      TAR_VAL_DAL & "," & TAR_VAL_AL & ",'" & volo_out & "'," & _
      "'" & volo_pr & "',NULL,NULL,'',NULL,NULL,NULL,NULL,NULL,'True','" & importo_prepagato & "','" & numero_giorni & "',NULL)"
            ...
            'SALVO LA RIGA DI CALCOLO E DI WARNING NELLE TABELLE PRENOTAZIONI_COSTI E PRENOTAZIONI_WARNING
            Dim sqlStr As String = "INSERT INTO prenotazioni_costi (id_documento, num_calcolo, ordine_stampa, id_gruppo, id_elemento, num_elemento, nome_costo," & _
            "selezionato,omaggiato, prepagato, valore_costo,valore_percentuale,imponibile,iva_imponibile,imponibile_scontato,iva_imponibile_scontato," & _
            "imponibile_onere, iva_onere, aliquota_iva,iva_inclusa,scontabile,omaggiabile,acquistabile_nolo_in_corso,id_a_carico_di,id_metodo_stampa,obbligatorio, franchigia_attiva, id_unita_misura, qta, packed,imponibile_scontato_prepagato,iva_imponibile_scontato_prepagato,imponibile_onere_prepagato,iva_onere_prepagato, commissioni_imponibile,sconto_su_imponibile_prepagato, commissioni_iva, commissioni_imponibile_originale, commissioni_iva_originale) " & _
            "(SELECT '" & id_prenotazione & "','1', ordine_stampa, id_gruppo,id_elemento,num_elemento,nome_costo," & _
            "selezionato,omaggiato,prepagato, valore_costo,valore_percentuale,imponibile,iva_imponibile,imponibile_scontato,iva_imponibile_scontato," & _
            ...
            Dim sqlPagamentoExtra As String = "INSERT INTO pagamenti_extra (Nr_Contratto,data,id_tippag, id_pos_funzioni_ares, importo, ID_STAZIONE, " & _
            "cassa, intestatario, scadenza, nr_aut," & _
            "tipsegno, DATACRE, UTECRE, N_PREN_RIF," & _
            " NR_BATCH,PER_IMPORTO, CONTABILIZZATO,TRANSATION_TYPE, CARD_TYPE, operazione_stornata, acquire_id, action_code, DATA_OPERAZIONE,TentativiPOS)" & _
             " VALUES " & _
            "('" & Nr_Contratto & "','" & funzioni_comuni.getDataDb_senza_orario(Now) & "','1011098650','4','0.00','" & stazione & "','','','','0000023','1',GetDate(),'web','" & num_prenotazione & "','000018'," & importo_prepagato & ",'0','MAG','2','0','00000000002','000','" & funzioni_comuni.getDataDb_senza_orario(Now) & "','1')"
            ...
            Dbc6.Open()
            Dim Cmd6 = New Data.SqlClient.SqlCommand(sqlPagamentoExtra, Dbc6)
            ...
            Dim sqlStrMaxId As String = "SELECT max(ID_CTR) FROM pagamenti_extra WITH(NOLOCK)"
            'Dim Sql3 = "SELECT MAX(Nr_Pren) FROM prenotazioni WITH(NOLOCK)"
            ...
            Dbc7.Open()
            Dim Cmd7 = New Data.SqlClient.SqlCommand(sqlStrMaxId, Dbc7)
